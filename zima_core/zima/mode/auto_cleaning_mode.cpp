...
AutoCleaningMode::Config::Config(const JsonSPtr& json)
...
AutoCleaningMode::AutoCleaningMode(
    ...
    : OperationMode("Auto cleaning mode", chassis, chassis_controller,
                    use_simple_slam),
      config_(new Config()),
      entrance_type_(entrance_type),
      cached_cleaning_info_(cached_cleaning_info),
  ...
  if ((entrance_type == EntranceType::kTotallyNewCleaning ||
       entrance_type == EntranceType::kNewCleaningBaseOnSelectedNavData) &&
...
AutoCleaning::AutoCleaningInfo::SPtr AutoCleaningMode::GetCleaningInfo() {
  ...
  auto cleaning_info = std::make_shared<AutoCleaning::AutoCleaningInfo>(
      auto_cleaning_->GetCleaningInfo());
...
void AutoCleaningMode::ReadyStateRun(
...
void AutoCleaningMode::InitializingStateRun(
    const OperationData::SPtr& operation_data) {
  // Initialize
  if (start_request_) {
...
void AutoCleaningMode::RunningStateRun(
    const OperationData::SPtr& operation_data) {
  if (start_request_) {
    ZWARN << "Start request will not be handled in this state.";
    start_request_ = false;
...
void AutoCleaningMode::PausedStateRun(
    const OperationData::SPtr& operation_data) {
  // Do nothing.
}

void AutoCleaningMode::HandlingExceptionStateRun(
    const OperationData::SPtr& operation_data) {}

void AutoCleaningMode::AutoFinishedStateRun(
    const OperationData::SPtr& operation_data) {}
...
void AutoCleaningMode::SwitchToRunningState(
    const OperationData::SPtr& operation_data) {
  ZGINFO << "Switch to running state.";
  state_ = State::kRunning;
}

void AutoCleaningMode::SwitchToPauseState(
    const OperationData::SPtr& operation_data) {
...
void AutoCleaningMode::SwitchToAutoFinishedState(
    const OperationData::SPtr& operation_data) {
...
void AutoCleaningMode::SwitchToStoppedState(
    const OperationData::SPtr& operation_data) {
...
bool AutoCleaningMode::InitializeForTotallyNewCleaning(
    const OperationData::SPtr& operation_data) {
...
bool AutoCleaningMode::InitializeForNewCleaningBaseOnSelectedNavData(
    const OperationData::SPtr& operation_data) {
  switch (initialize_state_) {
    case InitializeState::kNull: {
      auto_cleaning_.reset(new AutoCleaning(operation_data));
    ...
    case InitializeState::kInitForRelocation: {
      ...
      if (relocation_count_ > config_->relocation_count_max_) {
        ...
        if (relocation_count_ == 1) {
          ...
          ZGINFO << "Match with optimized degree "
                 << FloatToString(init_pose.Degree(), 2) << " for first match.";
    ...
    case InitializeState::kInitForSlam: {
      ...
      if (use_simple_slam_) {
        ...
        slam->UpdateProbabilityMap(
            operation_data->GetProbabilityIndexGridMap2DRef());
      ...
      if (FLAGS_debug_enable) {
        operation_data->GetNavMapConstRef()->GetRoomLayer()->Print(
            __FILE__, __FUNCTION__, __LINE__);
      }

      operation_data->SetStartPoint(current_pose);
      if (slam_wrapper_->StartSlam(current_pose,
                                   operation_data->GetSlamMapFileName(),
                                   operation_data->GetSlamMapFileName())) {
        initialize_state_ = InitializeState::kWaitForSlam;
        operate_on_slam_time_ = Time::Now();
...
bool AutoCleaningMode::InitializeForResumeCleaningWithRobotQuiescentState(
    const OperationData::SPtr& operation_data) {
  switch (initialize_state_) {
    case InitializeState::kNull: {
      ...
      auto_cleaning_.reset(
          new AutoCleaning(operation_data, cached_cleaning_info_));
      auto& event_manager = *EventManager::Instance();
...
bool AutoCleaningMode::InitializeForResumeCleaningWithRobotActiveState(
    const OperationData::SPtr& operation_data) {
  ZERROR << "Don't run here for now.";
  return false;
}

bool AutoCleaningMode::InitializeForSensor(
    const OperationData::SPtr& operation_data) {
  lidar_ready_ = false;
  odom_ready_ = false;
...
bool AutoCleaningMode::WaitForSensor(
    const OperationData::SPtr& operation_data) {
