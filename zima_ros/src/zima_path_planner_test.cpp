...
namespace zima_ros
{
    ...
    void ZimaNode::Run(const ZimaThreadWrapper::ThreadParam &param)
    {
        ...
        Steps steps = StepPoint::GenerateStepsBetweenTwoSteps(nav_map->GetFootStepLayer(), StepPoint(MapPoint(2, 2, 0)), StepPoint(MapPoint(2, -2, 0)));
        Steps steps2 = StepPoint::GenerateStepsBetweenTwoSteps(nav_map->GetFootStepLayer(), StepPoint(MapPoint(2, -2, 0)), StepPoint(MapPoint(-2, -2, 0)));
        Steps steps3 = StepPoint::GenerateStepsBetweenTwoSteps(nav_map->GetFootStepLayer(), StepPoint(MapPoint(-2, -2, 0)), StepPoint(MapPoint(-2, 2, 0)));
        Steps steps4 = StepPoint::GenerateStepsBetweenTwoSteps(nav_map->GetFootStepLayer(), StepPoint(MapPoint(-2, 2, 0)), StepPoint(MapPoint(2, 2, 0)));
        read_lock.Unlock();
        ...
        read_lock.Unlock();
        DynamicMapCellBound bound(bound_min.X(), bound_max.X(), bound_min.Y(), bound_max.Y());
        ...
        while (!shutdown_request_ && planner_manager.GeneratePath(nav_map, section_nav_map, current_pose, bound, 0, path, cell_path, true))
        {
