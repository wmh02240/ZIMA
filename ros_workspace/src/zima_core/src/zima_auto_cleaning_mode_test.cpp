...
void AutoCleaningModeEventWrapperTest() {
    ...
    auto auto_totally_new_cleaning = std::make_shared<AutoCleaningModeWrapper>(chassis, chassis_controller, slam1, nav_data);
...
void AutoCleaningModeTest() {
    ...
    auto chassis_controller = std::make_shared<ChassisController>(chassis);
    auto slam1 = std::make_shared<Slam1>();
    auto auto_totally_new_cleaning = std::make_shared<AutoCleaningMode>(chassis, chassis_controller, slam1);
    ...
    while (true) {
        ...
        if (auto_totally_new_cleaning->IsReady() || auto_totally_new_cleaning->IsAutoFinished()) {
            break;
        }
        Time::SleepMSec(20);
    }
...
void AutoScanHouseModeEventWrapperTest() {
    ...
    auto slam1 = std::make_shared<Slam1>();
    auto nav_data = std::make_shared<OperationData>();
    auto auto_scan_house = std::make_shared<AutoScanHouseModeWrapper>(chassis, chassis_controller, slam1, nav_data);
    // auto& event_manager = *EventManager::Instance();
