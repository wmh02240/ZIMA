...
void ProbabilityMapProbabilityElementsTest() {
    ...
    ProbabilityIndexGridMap2D::SPtr probability_map(new ProbabilityIndexGridMap2D("test map", 30, 30, NavMap::GetResolution(), config));
    ...
    for (; i < 300; i++) {
        ...
        if (FloatEqual(element.GetProbability(), config.max_probability_for_obstacle_in_cell_)) {
            break;
        }
    }
    ...
    for (; i < 300; i++) {
        ...
        if (FloatEqual(element.GetProbability(), config.min_probability_for_obstacle_in_cell_)) {
            break;
        }
    }
    ZERROR << "It takes " << i << " steps.";
