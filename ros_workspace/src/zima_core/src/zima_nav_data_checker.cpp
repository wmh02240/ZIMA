...
void CheckNavData(const NavData::SPtr &data) {
    ...
    data->GetOptimizedSlamValueGridMap2DRef()->Print(__FILE__, __FUNCTION__, __LINE__);
    ...
    data->GetNavMapRef()->GetUserBlockLayer()->Print(__FILE__, __FUNCTION__, __LINE__);
    ZINFO << "User select area map:";
    data->GetNavMapRef()->GetUserSelectAreaLayer()->Print(__FILE__, __FUNCTION__, __LINE__);
}
...
void CheckOperationData(const OperationData::SPtr &data) {
    ...
    if (path_size > 0) {
        ...
        data->GetNavMapRef()->GetPrintLayer()->Print(__FILE__, __FUNCTION__, __LINE__);
    }
