...
void PoseInterpolatorTest1() {
    ...
    pose_interpolator.AddData(std::make_shared<MergedOdomData>(1, MapPoint(-1, -1, 0)));
    pose_interpolator.AddData(std::make_shared<MergedOdomData>(1.5, MapPoint(1, 1, 60)));

    auto test_time = 0.3;
    ZINFO << DoubleToString(test_time, 1) << " IsInterpolatable: " << pose_interpolator.IsInterpolatable(test_time);
    test_time = 1.2;
    ZINFO << DoubleToString(test_time, 1) << " IsInterpolatable: " << pose_interpolator.IsInterpolatable(test_time);
    test_time = 1.8;
    ZINFO << DoubleToString(test_time, 1) << " IsInterpolatable: " << pose_interpolator.IsInterpolatable(test_time);
...
void PoseInterpolatorTest2() {
    PoseInterpolator::Config config;
    PoseInterpolator pose_interpolator(config);
    pose_interpolator.AddData(std::make_shared<MergedOdomData>(1, MapPoint(-1, -1, 0), MapPoint(0, 0, 0)));
    pose_interpolator.AddData(std::make_shared<MergedOdomData>(1.5, MapPoint(1, 1, 60), MapPoint(1, 1, 60)));
