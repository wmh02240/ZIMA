...
class CleaningRecord {
  public:
    ...
    CleaningRecord(const OperationData::SPtr &operation_data, const std::string &localize_path, const bool &localized = false);
    ~CleaningRecord();
    ...
    bool IsValid() const;
    bool Localize();
    static void ClearLocalization(const std::string &file_names, atomic_bool &localized_flag);
  ...
  public:
    ...
    class RecordSummary {
      public:
        ...
        RecordSummary(const uint32_t &start_timestamp, const OperationData::OperationType &type, const OperationData::OperationResult &result,
                      const float &clean_area_size, const uint32_t &duration)
            : start_timestamp_(start_timestamp), type_(type), result_(result), clean_area_size_(clean_area_size), duration_(duration) {}
    ...
    void RecheckAndTrimRecordsSummaryListFromRecordsData(const uint16_t &limit_size);
    ...
    static uint32_t ToUIntTimeStamp(const double &timestamp) { return static_cast<uint32_t>(floor(timestamp)); };
...
class RecordSummarySerializer {
  public:
    ...
    static ZimaProto::OperationData::PCleaningRecordSummary ToProto(const CleaningRecordManager::RecordSummary::SPtr &summary);
    static bool FromProto(CleaningRecordManager::RecordSummary::SPtr &summary, const ZimaProto::OperationData::PCleaningRecordSummary &proto);

    static ZimaProto::OperationData::PCleaningRecordSummaryList ToProto(const CleaningRecordManager::RecordSummaries &summaries);
    static bool FromProto(CleaningRecordManager::RecordSummaries &summaries, const ZimaProto::OperationData::PCleaningRecordSummaryList &proto);
};

class RecordSummaryListLoader : public LocalProtoFileReader {
  public:
    RecordSummaryListLoader() = default;
    explicit RecordSummaryListLoader(const std::string &file_dir, const std::string &file_name) : LocalProtoFileReader(file_dir, file_name){};
    ~RecordSummaryListLoader() = default;
...
class RecordSummaryListWriter : public LocalProtoFileWriter {
  public:
    ...
    explicit RecordSummaryListWriter(const std::string &file_dir, const std::string &file_name) : LocalProtoFileWriter(file_dir, file_name){};
    ~RecordSummaryListWriter() = default;

    bool WriteData(const CleaningRecordManager::RecordSummaries &data, const bool &is_binary);
};
