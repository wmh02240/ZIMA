...
const std::string LocalNavData::kSlamFileSuffix_ = ".local_nav_data_slam.pbstream";
const std::string LocalNavDataManager::kSelectedNavDataSuffix_ = ".selected";

LocalNavData::LocalNavData(const NavData::SPtr &nav_data, const std::string &localize_path, const bool &localized)
    ...
    if (nav_data == nullptr) {
        ...
        valid_.store(true);
        ZINFO << "Create nav_data(localized " << localized_.load() << " valid " << valid_.load() << ") index: " << static_cast<int>(nav_data_->GetIndex());
    }
}
...
bool LocalNavData::Localize() {
    ...
    if (localized_.load()) {
        ZWARN << "Nav data " << Time::DebugString(nav_data_->GetIndex()) << " has been localized.";
        return true;
    }
    ...
    const auto link_file_name = Time::DebugString(nav_data_->GetIndex()) + kSuffix_;
    ...
    if (!writter.WriteData(nav_data_, true)) {
        ClearLocalization(kLocalizePath_ + file_name + " " + kLocalizePath_ + link_file_name + " " + kLocalizePath_ + slam_file_name, localized_);
        ZWARN << "Nav data " << Time::DebugString(nav_data_->GetIndex()) << " localization failed.";
        return false;
    }

    if (!FileSystemHelper::IsFileOrDirectoryExists(kLocalizePath_ + link_file_name)) {
...
void LocalNavData::ClearLocalization(const std::string &file_names, atomic_bool &localized_flag) {
...
bool LocalNavDataManager::Config::ParseFromJson(const JsonSPtr &json) {
    ...
    if (!JsonHelper::GetUInt(*json, kLocalNavDataCountLimitKey_, local_nav_data_count_limit_)) {
...
LocalNavDataManager::LocalNavDataManager() : access_(std::make_shared<ReadWriteLock>()), kPreloadLocalNavDataIntoMemory_(false) {
    Config config;
    localize_path_ = config.config_valid_ ? config.local_nav_data_path_ : "/tmp/zima_local_nav_data/";
    nav_data_count_limit_ = config.config_valid_ ? config.local_nav_data_count_limit_ : 3;
    ...
    if (!FileSystemHelper::GetLinkFilesNamesInDirectory(localize_path_, link_file_names)) {
        ZERROR << "Directory unavailable.";
    } else {
        // for (auto&& name : link_file_names) {
        //   ZINFO << name;
        // }
        auto selected_file_it = find_if(link_file_names.begin(), link_file_names.end(),
                                        [](const std::string &file_name) -> bool { return StringEndsWith(file_name, kSelectedNavDataSuffix_); });
        ...
        if (selected_file_it != link_file_names.end()) {
            ...
            if (StringEndsWith(selected_file, LocalNavData::kSuffix_)) {
                ...
                selected_nav_data_index_ = static_cast<uint32_t>(atoi(selected_file.c_str()));
                ZGINFO << "Load selected nav_data as " << selected_nav_data_index_;
            } else {
...
bool LocalNavDataManager::AddNewNavData(const NavData::SPtr &nav_data) {
    ZINFO << "Add new nav_data.";
    LocalNavData::SPtr new_nav_data = std::make_shared<LocalNavData>(nav_data, localize_path_);
    ...
    if (kPreloadLocalNavDataIntoMemory_) {
        std::sort(local_nav_datas_.begin(), local_nav_datas_.end(), [](const LocalNavData::SPtr &a, const LocalNavData::SPtr &b) {
            return a->GetConstNavDataDataRef()->GetIndex() < b->GetConstNavDataDataRef()->GetIndex();
        });
        while (local_nav_datas_.size() >= nav_data_count_limit_) {
            for (auto local_nav_data_it = local_nav_datas_.begin(); local_nav_data_it != local_nav_datas_.end(); local_nav_data_it++) {
                const auto index = (*local_nav_data_it)->GetConstNavDataDataRef()->GetIndex();
                if (index != selected_nav_data_index_) {
                    ZINFO << "Remove newest unselected nav_data " << std::to_string(index);
                    lock.Unlock();
        ...
        std::sort(local_nav_data_indexs_.begin(), local_nav_data_indexs_.end(), [](const uint32_t &a, const uint32_t &b) { return a < b; });
        while (local_nav_data_indexs_.size() >= nav_data_count_limit_) {
            for (auto local_nav_data_it = local_nav_data_indexs_.begin(); local_nav_data_it != local_nav_data_indexs_.end(); local_nav_data_it++) {
        ...
        local_nav_data_indexs_.emplace_back(new_nav_data->GetConstNavDataDataRef()->GetIndex());
        std::sort(local_nav_data_indexs_.begin(), local_nav_data_indexs_.end(), [](const uint32_t &a, const uint32_t &b) { return a < b; });
    }
...
bool LocalNavDataManager::UpdateNavData(const NavData::SPtr &nav_data) {
    if (nav_data == nullptr) {
    ...
    if (kPreloadLocalNavDataIntoMemory_) {
        auto res = find_if(local_nav_datas_.begin(), local_nav_datas_.end(), [&](const LocalNavData::SPtr &local_nav_data) -> bool {
            if (local_nav_data == nullptr || !local_nav_data->IsValid()) {
                return false;
            }
            return local_nav_data->GetConstNavDataDataRef()->GetIndex() == nav_data_index;
        });
        ...
        auto res =
            find_if(local_nav_data_indexs_.begin(), local_nav_data_indexs_.end(), [&](const uint32_t &index) -> bool { return index == nav_data_index; });
...
bool LocalNavDataManager::RemoveNavData(const uint32_t &index) {
    ...
    auto clear_localization = [&]() -> void {
        ...
        const auto link_file_name = Time::DebugString(static_cast<double>(index)) + LocalNavData::kSuffix_;
        atomic_bool tmp;
        const auto slam_file_name = std::to_string(index) + LocalNavData::kSlamFileSuffix_;
        auto files = localize_path_ + file_name + " " + localize_path_ + link_file_name + " " + localize_path_ + slam_file_name;
        if (index == selected_nav_data_index_) {
    ...
    if (kPreloadLocalNavDataIntoMemory_) {
        ...
        auto res = find_if(local_nav_data_indexs_.begin(), local_nav_data_indexs_.end(), [&](const uint32_t &_index) -> bool { return _index == index; });
        if (res != local_nav_data_indexs_.end()) {
            clear_localization();
...
bool LocalNavDataManager::SelectNavData(const uint32_t &index) {
    ...
    auto select_func = [&]() -> bool {
        ...
        ZimaRunCommand(cmd);
        cmd = "ln -s " + localize_path_ + file_name + " " + localize_path_ + file_name + kSelectedNavDataSuffix_;
        if (ZimaRunCommand(cmd) != 0) {
...
LocalNavData::SPtr LocalNavDataManager::GetNavData(const uint32_t &index) {
    LocalNavData::SPtr ptr(nullptr);
    // ZINFO << (ptr == nullptr);
    ReadLocker lock(access_);
    if (kPreloadLocalNavDataIntoMemory_) {
        ...
        if (res != local_nav_data_indexs_.end()) {
            const auto file_name = std::to_string(index) + LocalNavData::kSuffix_;
            LocalNavData::SPtr nav_data = std::make_shared<LocalNavData>(nullptr, localize_path_);
            if (nav_data->LoadFromLocal(file_name) && nav_data->IsValid()) {
                ptr = nav_data;
            } else {
                ZERROR << "Index " << std::to_string(index) << " corresponding data invalid, remove it.";
                RemoveNavData(index);
            }
        } else {
            ZERROR << "Nav data for " << index << " is not founded.";
...
std::vector<uint32_t> LocalNavDataManager::GetNavDataIndexList() const {
    ...
    if (kPreloadLocalNavDataIntoMemory_) {
        for (auto &&local_nav_data : local_nav_datas_) {
            index_list.emplace_back(local_nav_data->GetConstNavDataDataRef()->GetIndex());
        }
    } else {
    ...
    for (auto &&name : names) {
        ...
        if (nav_data->LoadFromLocal(name) && nav_data->IsValid()) {
            ...
            if (kPreloadLocalNavDataIntoMemory_) {
                ...
                local_nav_data_indexs_.emplace_back(nav_data->GetConstNavDataDataRef()->GetIndex());
            }
        }
    }
    if (kPreloadLocalNavDataIntoMemory_) {
        ...
        ZGINFO << "Finish loading, loaded " << local_nav_datas_.size() << " nav_datas.";
    } else {
        std::sort(local_nav_data_indexs_.begin(), local_nav_data_indexs_.end(), [](const uint32_t &a, const uint32_t &b) { return a < b; });
        ZGINFO << "Finish loading, loaded " << local_nav_data_indexs_.size() << " nav_datas.";
    }
}
