...
bool OperationDataManager::CreateOperationData(OperationData::SPtr &operation_data, const SlamBase::SPtr &slam_wrapper,
                                               const OperationData::OperationType &operation_type, const bool &use_simple_slam) {
    ...
    if (operation_type == OperationData::OperationType::kAllHouseScanning) {
        ...
        operation_data = std::make_shared<OperationData>(OperationData::OperationType::kAllHouseScanning);
    } else {
        ...
        if (selected_operation_data_index != 0) {
            ...
            operation_data = std::make_shared<OperationData>(*selected_nav_data->GetNavDataCopyData());
            if (FLAGS_debug_enable) {
                operation_data->GetRawSlamValueGridMap2DRef()->Print(__FILE__, __FUNCTION__, __LINE__);
            }
...
bool OperationDataManager::ReleaseOperationData(OperationData::SPtr &operation_data, const SlamBase::SPtr &slam_wrapper, const bool &use_simple_slam) {
    if (operation_data == nullptr) {
    ...
    ZGINFO << "Index: " << std::to_string(index) << ", start time " << DoubleToString(start_time, 1);
    if (index == static_cast<uint32_t>(start_time)) {
        ZINFO << "New operation data.";
        auto init_room_map = std::make_shared<CharGridMap2D>(NavMap::kRoomMapName_, 1, 1, operation_data->GetOptimizedSlamValueGridMap2DRef()->GetResolution());
        MapConverter map_converter;
        map_converter.ConvertSlamValueGridMap2DToRoomMap(operation_data->GetOptimizedSlamValueGridMap2DRef(), init_room_map);
        ...
        if (room_map_util.UpdateRoomsInfo(init_room_map, init_rooms_info)) {
            ...
            if (operation_data->GetProbabilityIndexGridMap2DRef() != nullptr && operation_data->GetProbabilityIndexGridMap2DRef()->IsMarked()) {
        ...
        if (room_map_util.UpdateRoomsInfo(operation_data->GetNavMapConstRef()->GetRoomLayer(), new_rooms_info)) {
