...
OperationData::OperationData() : OperationData(OperationType::kAllHouseCleaning) {}

OperationData::OperationData(const OperationType &type)
    : NavData(), start_time_(Time::SystemNow()), operation_stop_watch_("Operation stop watch", false), operation_stage_(OperationStage::kOperating),
      operation_type_(type), operation_result_(OperationResult::kStopped), start_point_(MapPoint()), pause_world_pose_(nullptr), unsync_step_index_(0) {
    ...
    thread_param_ = zima::ZimaThreadWrapper::ThreadParam("Operation data internal process thread", zima::ZimaThreadManager::kMiscThreadIndex_, 100, 0.5, 1);
...
OperationData::OperationData(const bool &quiet)
    : NavData(quiet), start_time_(Time::SystemNow()), operation_stop_watch_("Operation stop watch", false), operation_stage_(OperationStage::kOperating),
      operation_type_(OperationType::kAllHouseCleaning), operation_result_(OperationResult::kStopped), start_point_(MapPoint()), pause_world_pose_(nullptr),
...
OperationData::OperationData(const NavData &ref)
    : NavData(ref), start_time_(Time::SystemNow()), operation_stop_watch_("Operation stop watch", false), operation_stage_(OperationStage::kOperating),
      operation_type_(OperationType::kAllHouseCleaning), operation_result_(OperationResult::kStopped), start_point_(MapPoint()), pause_world_pose_(nullptr),
      unsync_step_index_(0) {
    StepsRecorder::Config step_recorder_config;
    step_recorder_config.filter_distance_ = NavMap::GetResolution() * 2;
...
OperationData::OperationData(const OperationData &ref)
    : NavData(ref), start_time_(ref.start_time_), operation_stop_watch_(ref.operation_stop_watch_),
      steps_recorder_(std::make_shared<StepsRecorder>(*ref.steps_recorder_)), operation_stage_(ref.operation_stage_), operation_type_(ref.operation_type_),
      operation_result_(ref.operation_result_), start_point_(ref.start_point_), pause_world_pose_(ref.pause_world_pose_), thread_param_(ref.thread_param_),
...
void OperationData::RunThread() {
    if (thread_running_.load()) {
        ZINFO << "Thread for " << DoubleToString(start_time_, 3) << " is already running.";
        return;
    }
    stop_command_.store(false);

    auto thread_manager = zima::ZimaThreadManager::Instance();
    thread_manager->RegisterThread(std::thread(&OperationData::Thread, this, thread_param_), thread_param_);

    thread_running_.store(true);
    // Wait for thread up.
    Time::SleepMSec(100);
    ZGINFO << "Create operation thread for " << DoubleToString(start_time_, 3) << " success.";
}
...
void OperationData::StopThread() {
    ...
    ZGINFO << "Operation thread for " << DoubleToString(start_time_, 3) << " joined.";
    thread_running_.store(false);
}

void OperationData::PushNewSlamValueMap(const SlamValueGridMap2D::SPtr &slam_value_map) {
    WriteLocker lock(access_);
    cached_slam_map_deque_.emplace_back(slam_value_map);
    if (cached_slam_map_deque_.size() > 1) {
        ZWARN << "There are still " << (cached_slam_map_deque_.size() - 1) << " slam map to be processed.";
    }
}

bool OperationData::ProcessSlamValueMap(const SlamValueGridMap2D::SPtr &slam_value_map) {
    if (slam_value_map != nullptr) {
        ...
        auto optimized_slam_grid_map = std::make_shared<SlamValueGridMap2D>(*slam_value_map);
        lock.Unlock();
        ...
        CharGridMap2D::SPtr char_slam_grid_map = std::make_shared<CharGridMap2D>("char slam map", 1, 1, optimized_slam_grid_map->GetResolution());
        MapConverter map_converter;
        map_converter.ConvertSlamValueGridMap2DToCharSlamMap(optimized_slam_grid_map, char_slam_grid_map);
...
void OperationData::AddStep(const StepPoint &step_point) {
    ...
    if (steps_recorder_->AddPathPoint(step_point)) {
        ...
        ZGINFO << step_point.DebugString() << ", " << step_cell.DebugString() << " " << steps_recorder_->GetPathSize();
    }
}
...
void OperationData::UpdateUserSelectAreaInNavMap(const CharGridMap2D::SPtr &new_user_select_area_map) {
...
void OperationData::SetOperationStage(const OperationData::OperationStage &value) {
    WriteLocker lock(access_);
...
void OperationData::SetOperationResult(const OperationData::OperationResult &value) {
    WriteLocker lock(access_);
...
std::string OperationData::DebugOperationInfo() const {
    ...
    std::string str = "\nStart time: " + Time::DebugString(GetStartTime()) + "(" + DoubleToString(GetStartTime(), 1) + ")" +
                      "\nCover area: " + FloatToString(area, 2) + "m2\nOperated seconds:" + std::to_string(floor(GetDuration())) + "s (" +
                      FloatToString(GetDuration() / 60, 2) + "min)\nEfficiency: " + std::to_string(area / GetDuration() * 60) + "m2/min.";
    return str;
}
...
ZimaProto::OperationData::POperationData OperationDataSerializer::ToProto(const OperationData::SPtr &data) {
...
bool OperationDataSerializer::FromProto(OperationData::SPtr &data, const ZimaProto::OperationData::POperationData &proto) {
    if (data == nullptr) {
        ZERROR << "Data empty.";
        return false;
    }
    ...
    for (auto &&step : proto.step_points()) {
        ...
        case ZimaProto::OperationData::PStepPointType::kPauseStep: {
            data->steps_recorder_->AddPathPoint(
                StepPoint(MapPoint(step.point().x(), step.point().y(), step.point().degree()), nullptr, StepPoint::Type::kPauseStep), true, true);
            break;
        }
        case ZimaProto::OperationData::PStepPointType::kNormalStep:
        default: {
            data->steps_recorder_->AddPathPoint(
                StepPoint(MapPoint(step.point().x(), step.point().y(), step.point().degree()), nullptr, StepPoint::Type::kNormalStep), true, true);
            break;
        }
        }
    }

    data->operation_stop_watch_ = StopWatch(data->operation_stop_watch_.Name(), proto.duration());
    ...
    case ZimaProto::OperationData::POperationResult::kFinishAutoCleaning: {
        data->operation_result_ = OperationData::OperationResult::kFinishAutoCleaning;
        break;
    }
...
bool OperationDataLoader::LoadData(OperationData::SPtr &data, const bool &quiet) {
    ZimaProto::OperationData::POperationData proto;
...
bool OperationDataWriter::WriteData(const OperationData::SPtr &data, const bool &is_binary) {
