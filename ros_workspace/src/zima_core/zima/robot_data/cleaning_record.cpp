...
const std::string CleaningRecordManager::kRecordListFileName_ = "record_list.pb.txt";

CleaningRecord::CleaningRecord(const OperationData::SPtr &operation_data, const std::string &localize_path, const bool &localized)
    : access_(std::make_shared<ReadWriteLock>()), localized_(localized), kLocalizePath_(localize_path) {
...
bool CleaningRecord::Localize() {
    ...
    if (localized_.load()) {
        ZWARN << "Record " << Time::DebugString(record_data_->GetStartTime()) << " has been localized.";
        return true;
    }
    ...
    const auto link_file_name = Time::DebugString(record_data_->GetStartTime()) + kSuffix_;

    OperationDataWriter writter(kLocalizePath_, file_name);
    if (!writter.WriteData(record_data_, true)) {
        ClearLocalization(kLocalizePath_ + file_name + " " + kLocalizePath_ + link_file_name, localized_);
        localized_.store(false);
        ZWARN << "Record " << Time::DebugString(record_data_->GetStartTime()) << " localization failed.";
        return false;
    }

    std::string cmd = "ln -s " + kLocalizePath_ + file_name + " " + kLocalizePath_ + link_file_name;
...
void CleaningRecord::ClearLocalization(const std::string &file_names, atomic_bool &localized_flag) {
...
std::string CleaningRecordManager::RecordSummary::DebugString() {
    std::string str = "\nStart time: " + Time::DebugString(static_cast<double>(start_timestamp_)) + "(" + std::to_string(start_timestamp_) + ")";
    str += "\nType: ";
    switch (type_) {
    ...
    str += "\nCleaning area: " + FloatToString(clean_area_size_, 2) + "m2\nCleaning seconds:" + std::to_string(duration_) + "s (" +
...
bool CleaningRecordManager::Config::ParseFromJson(const JsonSPtr &json) {
    ...
    // Override.
    if (!JsonHelper::GetString(*json, kCleaningRecordPathKey_, cleaning_record_path_)) {
        ZERROR << "Config " << kCleaningRecordPathKey_ << " not found.";
        return false;
    }

    if (!JsonHelper::GetUInt(*json, kCleaningRecordCountLimitKey_, cleaning_record_count_limit_)) {
...
CleaningRecordManager::CleaningRecordManager() : access_(std::make_shared<ReadWriteLock>()) {
    Config config;
    localize_path_ = config.config_valid_ ? config.cleaning_record_path_ : "/tmp/zima_clean_record/";
    record_count_limit_ = config.config_valid_ ? config.cleaning_record_count_limit_ : 20;
...
CleaningRecordManager::RecordSummaries CleaningRecordManager::GetRecordsSummaryList() const {
    ReadLocker lock(access_);
    return record_summaries_;
}

bool CleaningRecordManager::LocalizeRecordsSummaryList() {
    ZGINFO << "Start updating summary list file \"" << localize_path_ << kRecordListFileName_;
    ReadLocker lock(access_);
    ...
    if (!writter.WriteData(record_summaries_, false)) {
        ...
        ZWARN << "Record summary list file \"" << localize_path_ << kRecordListFileName_ << "\" localization failed.";
        return false;
    }

    ZGINFO << "Update summary list file \"" << localize_path_ << kRecordListFileName_ << "\" succeed.";
    return true;
}
...
bool CleaningRecordManager::LoadRecordsSummaryList() {
    ...
    if (!loader.LoadData(summeries)) {
        ZWARN << "Load for summary list file \"" << localize_path_ << kRecordListFileName_ << "\" failed.";
        return false;
    }
    ...
    std::sort(record_summaries_.begin(), record_summaries_.end(),
              [](const RecordSummary::SPtr &a, const RecordSummary::SPtr &b) { return a->start_timestamp_ > b->start_timestamp_; });
    ZGINFO << "Finish loading, loaded " << record_summaries_.size() << " summaries.";
    return true;
}
...
void CleaningRecordManager::ClearRecordsSummaryListLocalization() {
    ...
    if (ZimaRunCommand(cmd) != 0) {
        ZWARN << "Clear for Record summary list \"" << localize_path_ + kRecordListFileName_ << "\" failed.";
        return;
    }

    ZGINFO << "Clear for Record summary list \"" << localize_path_ + kRecordListFileName_ << "\" succeed.";
}

void CleaningRecordManager::RecheckAndTrimRecordsSummaryListFromRecordsData(const uint16_t &limit_size) {
    ...
    std::sort(record_names.begin(), record_names.end(), [](const std::string &a, const std::string &b) { return a > b; });
    ...
    for (auto &&name : record_names) {
        if (record_count < limit_size) {
            CleaningRecord::SPtr record = std::make_shared<CleaningRecord>(nullptr, localize_path_);
            if (record->LoadFromLocal(name) && record->IsValid()) {
                new_summaries.emplace_back(std::make_shared<CleaningRecordManager::RecordSummary>(
                    ToUIntTimeStamp(record->GetConstRecordDataRef()->GetStartTime()), record->GetConstRecordDataRef()->GetOperationType(),
                    record->GetConstRecordDataRef()->GetOperationResult(), record->GetConstRecordDataRef()->GetNavMapConstRef()->GetStepAreaSize(),
                    ToUIntTimeStamp(record->GetConstRecordDataRef()->GetDuration())));
                ZGINFO << "Check valid summary: " << new_summaries.back()->DebugString();
                record_count++;
            } else {
    ...
    std::sort(record_summaries_.begin(), record_summaries_.end(),
              [](const RecordSummary::SPtr &a, const RecordSummary::SPtr &b) { return a->start_timestamp_ > b->start_timestamp_; });
    ZGINFO << "Finish re-checking, " << record_summaries_.size() << " valid records.";
    lock.Unlock();
    LocalizeRecordsSummaryList();
}

bool CleaningRecordManager::AddNewRecord(const OperationData::SPtr &operation_data) {
    ZGINFO << "Add new record.";
    CleaningRecord::SPtr new_record = std::make_shared<CleaningRecord>(operation_data, localize_path_);
    ...
    WriteLocker lock(access_);
    record_summaries_.emplace_back(std::make_shared<CleaningRecordManager::RecordSummary>(
        ToUIntTimeStamp(new_record->GetConstRecordDataRef()->GetStartTime()), new_record->GetConstRecordDataRef()->GetOperationType(),
        new_record->GetConstRecordDataRef()->GetOperationResult(), new_record->GetConstRecordDataRef()->GetNavMapConstRef()->GetStepAreaSize(),
        ToUIntTimeStamp(new_record->GetConstRecordDataRef()->GetDuration())));
    ZGINFO << "Add new summary: " << record_summaries_.back()->DebugString();
    std::sort(record_summaries_.begin(), record_summaries_.end(),
              [](const RecordSummary::SPtr &a, const RecordSummary::SPtr &b) { return a->start_timestamp_ > b->start_timestamp_; });
    ZINFO << "Add new record " << new_record->GetConstRecordDataRef()->GetIndex() << " succeed, total " << record_summaries_.size() << " records.";
...
CleaningRecordManager::RecordSummary::SPtr CleaningRecordManager::GetRecordSummary(const uint32_t &start_timestamp) {
    ReadLocker lock(access_);
    RecordSummary::SPtr ptr(nullptr);
    auto res = find_if(record_summaries_.begin(), record_summaries_.end(), [&](const RecordSummary::SPtr &summary) -> bool {
        if (summary == nullptr) {
            return false;
        }
...
CleaningRecord::SPtr CleaningRecordManager::GetRecordData(const uint32_t &start_timestamp) {
    ...
    const auto file_name = std::to_string(start_timestamp) + CleaningRecord::kSuffix_;
...
bool CleaningRecordManager::RemoveRecords(const std::vector<uint32_t> &start_timestamps) {
    ...
    for (auto &&start_timestamp : start_timestamps) {
        ZGINFO << "Remove record " << Time::DebugString(static_cast<double>(start_timestamp)) << "(" << start_timestamp << ").";
        {
            const auto file_name = std::to_string(start_timestamp) + CleaningRecord::kSuffix_;
            const auto link_file_name = Time::DebugString(static_cast<double>(start_timestamp)) + CleaningRecord::kSuffix_;
            atomic_bool tmp;
            CleaningRecord::ClearLocalization(localize_path_ + file_name + " " + localize_path_ + link_file_name, tmp);
        }
        {
            auto res = find_if(record_summaries_.begin(), record_summaries_.end(), [&](const RecordSummary::SPtr &summary) -> bool {
                if (summary == nullptr) {
            ...
            if (res != record_summaries_.end()) {
                ...
                ZGINFO << "Remove record " << Time::DebugString(static_cast<double>(start_timestamp)) << "(" << start_timestamp << ") succeed, total "
...
bool CleaningRecordManager::TrimRecords(const uint16_t &limit_size) {
    ...
    std::sort(record_summaries_.begin(), record_summaries_.end(),
              [](const RecordSummary::SPtr &a, const RecordSummary::SPtr &b) { return a->start_timestamp_ > b->start_timestamp_; });
    ZGINFO << "Finish trimming records, " << record_summaries_.size() << " valid records.";
    write_lock.Unlock();
    LocalizeRecordsSummaryList();
...
ZimaProto::OperationData::PCleaningRecordSummary RecordSummarySerializer::ToProto(const CleaningRecordManager::RecordSummary::SPtr &summary) {
    ...
    case OperationData::OperationType::kSelectRoomCleaning: {
        proto.set_operation_type(ZimaProto::OperationData::POperationType::kSelectRoomCleaning);
        break;
    }
    case OperationData::OperationType::kSelectAreaCleaning: {
        proto.set_operation_type(ZimaProto::OperationData::POperationType::kSelectAreaCleaning);
        break;
    }
    case OperationData::OperationType::kAllHouseScanning: {
        proto.set_operation_type(ZimaProto::OperationData::POperationType::kAllHouseScanning);
        break;
    }
    case OperationData::OperationType::kAllHouseCleaning:
    default: {
        proto.set_operation_type(ZimaProto::OperationData::POperationType::kAllHouseCleaning);
        break;
    }
    }
    switch (summary->result_) {
    case OperationData::OperationResult::kFinishAutoCleaning: {
        proto.set_operation_result(ZimaProto::OperationData::POperationResult::kFinishAutoCleaning);
        break;
    }
    case OperationData::OperationResult::kStopped:
    default: {
        proto.set_operation_result(ZimaProto::OperationData::POperationResult::kStopped);
        break;
    }
    }
...
bool RecordSummarySerializer::FromProto(CleaningRecordManager::RecordSummary::SPtr &summary, const ZimaProto::OperationData::PCleaningRecordSummary &proto) {
    if (summary == nullptr) {
...
ZimaProto::OperationData::PCleaningRecordSummaryList RecordSummarySerializer::ToProto(const CleaningRecordManager::RecordSummaries &summaries) {
...
bool RecordSummarySerializer::FromProto(CleaningRecordManager::RecordSummaries &summaries, const ZimaProto::OperationData::PCleaningRecordSummaryList &proto) {
    summaries.clear();
    for (auto &&summary_proto : proto.record_summary_list()) {
        auto summary_ptr = std::make_shared<CleaningRecordManager::RecordSummary>(0, OperationData::OperationType::kAllHouseCleaning,
...
bool RecordSummaryListLoader::LoadData(CleaningRecordManager::RecordSummaries &data) {
    ZimaProto::OperationData::PCleaningRecordSummaryList proto;
...
bool RecordSummaryListWriter::WriteData(const CleaningRecordManager::RecordSummaries &data, const bool &is_binary) {
