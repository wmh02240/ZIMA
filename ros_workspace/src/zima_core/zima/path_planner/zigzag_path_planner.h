...
class ZigZagPlanner : public PlannerBase {
  public:
     ...
     */
    virtual bool GeneratePath(NavMap::SPtr map, const MapPoint &start_pose, const DynamicMapCellBound &bound, const CharGridMap2D::DataType &current_room_index,
                              ZigZagPath &output_path, MapCellPath &output_cell_path, const bool &use_x_direction) = 0;

  protected:
    uint32_t CountUncleanedCell(NavMap::SPtr map, const MapCell &start_pose, const DynamicMapCellBound &bound,
...
class CurrentLinePlanner : public ZigZagPlanner {
  public:
    ...
    CurrentLinePlanner(const bool &positive_direction, const unsigned int &half_line_width) {
  ...
  protected:
     ...
     */
    bool GeneratePath(NavMap::SPtr map, const MapPoint &start_pose, const DynamicMapCellBound &bound, const CharGridMap2D::DataType &current_room_index,
                      ZigZagPath &output_path, MapCellPath &output_cell_path, const bool &use_x_direction) override;
...
class NextLinePlanner : public ZigZagPlanner {
  public:
    ...
    NextLinePlanner(const bool &positive_direction, const unsigned int &half_line_width) {
        positive_direction_ = positive_direction;
        half_line_width_ = half_line_width;
        ZGINFO << (positive_direction_ ? "Positive" : "Negative");
    };
...
class ZigZagPlannerManager : public ZigZagPlanner {
  public:
    ...
    bool GeneratePath(NavMap::SPtr map, NavMap::SPtr section_map, const MapPoint &start_pose, const DynamicMapCellBound &bound,
                      const CharGridMap2D::DataType &current_room_index, ZigZagPath &output_path, MapCellPath &output_cell_path, const bool &use_x_direction);
