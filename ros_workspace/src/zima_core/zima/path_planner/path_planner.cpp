...
bool PlannerBase::InflactObstacleMap(NavMap::SPtr process_map, const DynamicMapCellBound &bound, const MapCell &start_cell,
                                     const uint16_t &inflate_max_iteration_count, const ExpendCondition &extra_expend_cond,
    ...
    ExpendCondition sensor_expend_cond = [&](MultiLayersCharGridMap2D::SPtr _map, MapCell &curr_step, MapCell &next_step) {
    ...
    ExpendCondition user_block_expend_cond = [&](MultiLayersCharGridMap2D::SPtr _map, MapCell &curr_step, MapCell &next_step) {
        // Dont expend to robot.
        if (next_step == start_cell) {
            return false;
        }
    ...
    if (!obstacle_cells.empty()) {
        if (!InflateObstaclesInMap(process_map, NavMap::kSensorMapName_, inflate_max_iteration_count, obstacle_cells, sensor_expend_cond,
    ...
    if (!user_block_cells.empty()) {
        if (!InflateObstaclesInMap(process_map, NavMap::kUserBlockMapName_, inflate_max_iteration_count, user_block_cells, user_block_expend_cond,
                                   enable_square_inflaction)) {
            ZWARN;
            return false;
        }
...
bool PlannerBase::InflactSlamObstacleMap(NavMap::SPtr process_map, const DynamicMapCellBound &bound, const MapCell &start_cell,
                                         const uint16_t &inflate_max_iteration_count, const ExpendCondition &extra_expend_cond,
                                         const bool &enable_square_inflaction, const bool &print_result) {
    ...
    ExpendCondition slam_expend_cond = [&](MultiLayersCharGridMap2D::SPtr _map, MapCell &curr_step, MapCell &next_step) {
        // Dont expend to robot.
        if (next_step == start_cell) {
            return false;
        }
    ...
    if (!slam_wall_cells.empty()) {
        if (!InflateObstaclesInMap(process_map, NavMap::kSlamMapName_, inflate_max_iteration_count, slam_wall_cells, slam_expend_cond,
                                   enable_square_inflaction)) {
            ZWARN;
            return false;
        }
...
bool PlannerBase::InflateObstaclesInMap(NavMap::SPtr map, const std::string &map_type, const uint16_t &inflate_max_iteration_count, const MapCells &start_cells,
    ...
    if (!target_map->GetValue(start_cells.front().X(), start_cells.front().Y(), obs_value)) {
        ZWARN << start_cells.front().DebugString() << " is not in " << map_type << " layer.";
        return false;
    }
    ...
    if (enable_square_inflaction) {
        ...
        auto square_inflate = [&](const MapCell &cell) {
            for (auto i = -NavMap::kRobotCellWidth_2_; i <= NavMap::kRobotCellWidth_2_; i++) {
                for (auto j = -NavMap::kRobotCellWidth_2_; j <= NavMap::kRobotCellWidth_2_; j++) {
...
void PlannerBase::PrintPathInMap(NavMap::SPtr map, const DynamicMapCellBound &print_bound, const MapCellPath &path) {
    // Print for path.
    ...
    CharGridMap2D::OverridePrintCellFunc print_func = [](const CharGridMap2D::DataType &value) {
        std::string str;
        if (value == 0) {
            str = ".";
...
void PlannerBase::PrintPathInMap(NavMap::SPtr map, const MapPointPath &path) { PrintPathInMap(map, map->GetPrintLayer()->GetDataBound(), path); }

void PlannerBase::PrintPathInMap(NavMap::SPtr map, const DynamicMapCellBound &print_bound, const MapPointPath &path) {
...
bool PlannerBase::OptimizePathToShortest(NavMap::SPtr map, const MapCells &input_cell_path, MapPointPath &output_point_path, MapCellPath &output_cell_path,
                                         const CharGridMap2D::DataType &current_room_index, const bool &consider_slam_map) {
    ...
    InflactObstacleMap(
        process_map, data_bound, input_cell_path.front(), NavMap::kRobotCellWidth_2_ / 2,
        [](MultiLayersCharGridMap2D::SPtr map, MapCell &curr_step, MapCell &next_step) { return true; }, false);
    if (consider_slam_map) {
    ...
    auto check_near_user_block_area = [&](const MapCell &cell) {
        ...
        for (auto x = -1; x <= 1; x++) {
            for (auto y = -1; y <= 1; y++) {
                user_block_layer->GetValue(cell.X() + x, cell.Y() + y, user_block_value);
    ...
    while (true) {
        ...
        while (*target_it != *straight_end_it) {
            ...
            auto tmp_cells = footstep_layer->GenerateCellsBetweenTwoCells(*start_it, *target_it);
            ...
            if (start_from_end) {
                for (auto it = tmp_cells.end() - 1;;) {
                    ...
                    if (!process_map->IsClearedInSensorMap(*it) || check_near_room_edge(*it) || check_near_user_block_area(*it) || check_near_slam_wall(*it)) {
                        path_available = false;
                        if (std::distance(tmp_cells.begin(), it) < static_cast<int>(tmp_cells.size() / 2)) {
    ...
    output_cell_path.clear();
    {
        ...
        while (true) {
            ...
            output_cell_path.insert(output_cell_path.end(), _cells.begin(), _cells.end() - 1);

            MapPoint p1, p2;
            footstep_layer->MapToWorld(*it1, p1);
            footstep_layer->MapToWorld(*it2, p2);
            // ZINFO;
            tmp_path = GenerateInterpolationPoints(p1, p2, process_map->GetResolution() * 0.99);
            output_point_path.insert(output_point_path.end(), tmp_path.begin(), tmp_path.end() - 1);
...
bool PlannerBase::GeneratePathToTarget(NavMap::SPtr process_map, const MapPoint &start_pose, const MapPoint &target_point, MapPointPath &output_path,
                                       MapCellPath &output_cell_path, const ExpendCondition &additional_expend_cond) {
    auto sensor_map = process_map->GetSensorLayer();
    ReadLocker sensor_map_lock(sensor_map->GetLock());
    ...
    MapCell null_cell;
    ZGINFO << typeid(*this).name() << " Start from: " << start_cell.DebugString() << ", target: " << target_cell.DebugString();

    // sensor_map->Print(__FILE__, __FUNCTION__, __LINE__);

    if (Search(process_map, start_cells, null_cell, expend_cond, finish_cond, step_cb, iter_step_cb)) {
        ...
        if (OptimizePathToShortest(process_map, output_cell_path, optimized_point_path, optimized_cell_path)) {
