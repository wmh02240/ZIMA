...
bool SwitchSectionPlanner::GeneratePath(NavMap::SPtr map, const MapPoint &start_pose, const CharGridMap2D::DataType &current_room_index,
                                        MapPointPath &output_path, MapCellPath &output_cell_path, const bool &consider_slam_map) {
    bool found = false;
    MapCell start_cell;
    ...
    MapCell target_cell;
    ZGINFO << typeid(*this).name() << " Start from: " << start_cell.DebugString() << ", consider slam map " << std::to_string(consider_slam_map);
...
bool SwitchRoomPlanner::GeneratePathToSpecificRoom(NavMap::SPtr map, const MapPoint &start_pose, const CharGridMap2D::DataType &next_room_index,
                                                   MapPointPath &output_path, MapCellPath &output_cell_path, const bool &consider_slam_map) {
    bool found = false;
    MapCell start_cell;
    if (!map->GetFootStepLayer()->WorldToMap(start_pose, start_cell)) {
...
bool SwitchRoomPlanner::GeneratePathToNearestRoom(NavMap::SPtr map, const MapPoint &start_pose, MapPointPath &output_path, MapCellPath &output_cell_path,
