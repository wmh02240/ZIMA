...
StandbyModeWrapper::StandbyModeWrapper(const Chassis::SPtr &chassis, const ChassisController::SPtr &chassis_controller, const SlamBase::SPtr &slam_wrapper)
    : ModeEventWrapperBase(ModeLabel::kStandbyMode) {
    standby_mode_ = std::make_shared<StandbyMode>(chassis, chassis_controller, slam_wrapper);
    mode_ = standby_mode_;
    standby_mode_->Start();
...
void StandbyModeWrapper::Run(OperationData::SPtr &operation_data, const SlamBase::SPtr &slam_wrapper) {
...
void StandbyModeWrapper::InitializeForCallBack() {
    ...
    RegisterUserEventCallBack(KeyboardEvent::kLabel_, [this](const UserEvent::SPtr &user_event, const OperationData::SPtr &operation_data) -> bool {
        ...
        auto local_nav_data_index_list = local_nav_data_manager->GetNavDataIndexList();
        ...
        case '2': {
            ...
            if (local_nav_data_index_list.size() > static_cast<uint>(index)) {
                local_nav_data_manager->SelectNavData(local_nav_data_index_list.at(index));
            } else {
                ZINFO << "Null option.";
            }
            break;
        ...
        case '6': {
            ...
            if (local_nav_data_index_list.size() > static_cast<uint>(index)) {
                local_nav_data_manager->RemoveNavData(local_nav_data_index_list.at(index));
            } else {
                ZINFO << "Null option.";
            }
            break;
        ...
        case '9': {
            ...
            if (local_nav_data_index_list.size() > static_cast<uint>(index)) {
                auto nav_data = local_nav_data_manager->GetNavData(local_nav_data_index_list.at(index));
                ZINFO << "Index: " << std::to_string(local_nav_data_index_list.at(index));
                nav_data->GetConstNavDataDataRef()->GetOptimizedSlamValueGridMap2DRef()->Print(__FILE__, __FUNCTION__, __LINE__);
                auto virtual_walls = nav_data->GetConstNavDataDataRef()->GetAllVirtualWall();
                for (auto &&virtual_wall : virtual_walls) {
                    ZINFO << "Virtual wall:" << virtual_wall.second->DebugString(nav_data->GetConstNavDataDataRef()->GetNavMapConstRef()->GetPrintLayer());
                }
                auto block_areas = nav_data->GetConstNavDataDataRef()->GetAllBlockArea();
                for (auto &&block_area : block_areas) {
                    ZINFO << "Block area:" << block_area.second->DebugString(nav_data->GetConstNavDataDataRef()->GetNavMapConstRef()->GetPrintLayer());
                }
            } else {
                ZINFO << "Null option.";
            }
    ...
    RegisterUserEventCallBack(LocalNavDataEvent::kLabel_, [this](const UserEvent::SPtr &user_event, const OperationData::SPtr &operation_data) -> bool {
        auto local_nav_data_event = dynamic_cast<const LocalNavDataEvent *>(user_event.get());
        auto local_nav_data_manager = LocalNavDataManager::Instance();
    ...
    RegisterUserEventCallBack(VirtualWallBlockAreaEvent::kLabel_, [this](const UserEvent::SPtr &user_event, const OperationData::SPtr &operation_data) -> bool {
        ...
        if (_operation_data == nullptr) {
            ...
            if (selected_operation_data_index != 0) {
                ...
                _operation_data = std::make_shared<OperationData>(*selected_nav_data->GetNavDataCopyData());
            } else {
        ...
        for (auto &&operation : virtual_wall_block_area_event->operations_) {
            if (operation.data_type_ == VirtualWallBlockAreaEvent::Operation::DataType::kVirtualWall) {
                ...
                default: {
                    _operation_data->UpdateVirtualWall(operation.index_, virtual_wall);
                    operated = true;
                    break;
                }
                }
...
void StandbyModeWrapper::PrintInfo() {
    ...
    auto local_nav_data_index_list = local_nav_data_manager->GetNavDataIndexList();
    auto selected_nav_data_index = local_nav_data_manager->GetSelectedNavDataIndex();
    std::string info_str;
    ...
    info_str += "\n-------------------------------------";
    {
        ...
        for (auto &&index : local_nav_data_index_list) {
            info_str += "\n" + std::to_string(num) + ": Select nav data " + std::to_string(index) + "(" + Time::DebugString(index) + ")";
        ...
        for (auto &&index : local_nav_data_index_list) {
            info_str += "\n" + std::to_string(num) + ": Remove nav data " + std::to_string(index) + "(" + Time::DebugString(index) + ")";
            if (selected_nav_data_index == index) {
                info_str += "(selected)";
            }
            num++;
        ...
        for (auto &&index : local_nav_data_index_list) {
            info_str += "\n" + std::to_string(num) + ": Print info in nav data " + std::to_string(index) + "(" + Time::DebugString(index) + ")";
            if (selected_nav_data_index == index) {
                info_str += "(selected)";
            }
            num++;
