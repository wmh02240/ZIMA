...
AutoScanHouseMode::Config::Config(const JsonSPtr &json) : OperationMode::Config(json) {
    if (json == nullptr) {
        JsonSPtr config;
...
AutoScanHouseMode::AutoScanHouseMode(const Chassis::SPtr &chassis, const ChassisController::SPtr &chassis_controller, const SlamBase::SPtr &slam_wrapper,
                                     const EntranceType &entrance_type, const AutoScanHouse::AutoScanningInfo::SPtr &cached_scanning_info)
    : OperationMode("Auto scanning mode", chassis, chassis_controller), config_(new Config()), entrance_type_(entrance_type),
      cached_scanning_info_(cached_scanning_info), slam_wrapper_(slam_wrapper) {
...
AutoScanHouse::AutoScanningInfo::SPtr AutoScanHouseMode::GetScanningInfo() {
    ...
    auto scanning_info = std::make_shared<AutoScanHouse::AutoScanningInfo>(auto_scan_house_->GetScanningInfo());
...
void AutoScanHouseMode::ReadyStateRun(const OperationData::SPtr &operation_data) {
    // Wait for event.
    if (start_request_) {
...
void AutoScanHouseMode::InitializingStateRun(const OperationData::SPtr &operation_data) {
    // Initialize
    if (start_request_) {
...
void AutoScanHouseMode::RunningStateRun(const OperationData::SPtr &operation_data) {
    if (start_request_) {
        ZWARN << "Start request will not be handled in this state.";
        start_request_ = false;
    }
...
void AutoScanHouseMode::PausedStateRun(const OperationData::SPtr &operation_data) {
    // Do nothing.
}

void AutoScanHouseMode::HandlingExceptionStateRun(const OperationData::SPtr &operation_data) {}

void AutoScanHouseMode::AutoFinishedStateRun(const OperationData::SPtr &operation_data) {}
...
void AutoScanHouseMode::SwitchToRunningState(const OperationData::SPtr &operation_data) {
    ZGINFO << "Switch to running state.";
    state_ = State::kRunning;
}

void AutoScanHouseMode::SwitchToPauseState(const OperationData::SPtr &operation_data) {
...
void AutoScanHouseMode::SwitchToAutoFinishedState(const OperationData::SPtr &operation_data) {
    ZGINFO << "Switch to auto finished state.";
    state_ = State::kAutoFinished;
    operation_data->SetOperationResult(OperationData::OperationResult::kFinishAutoScanHouse);
    operation_data->GetStopWatchRef().Stop();
    operate_on_slam_time_ = Time::Now();
}

void AutoScanHouseMode::SwitchToStoppedState(const OperationData::SPtr &operation_data) {
...
bool AutoScanHouseMode::InitializeForNewScanning(const OperationData::SPtr &operation_data) {
...
bool AutoScanHouseMode::InitializeForResumeScanning(const OperationData::SPtr &operation_data) {
    switch (initialize_state_) {
    case InitializeState::kNull: {
        if (cached_scanning_info_ != nullptr) {
            ZGINFO << "Resume auto scanning for cached info.";
        }
        auto_scan_house_.reset(new AutoScanHouse(operation_data, cached_scanning_info_));
        auto &event_manager = *EventManager::Instance();
...
bool AutoScanHouseMode::InitializeForSensor(const OperationData::SPtr &operation_data) {
    lidar_ready_ = false;
    odom_ready_ = false;
...
bool AutoScanHouseMode::WaitForSensor(const OperationData::SPtr &operation_data) {
