...
bool DijkstraBase::Search(MultiLayersCharGridMap2D::SPtr map, const MapCells &start_cells, MapCell &target, const ExpendCondition &expend_cond,
                          const FinishCondition &finish_cond, const CellStepCallback &step_cb, const IterationStepCallback &iter_step_cb) {
    bool found = false;
    ...
    cost_map_.ResetMap(map->GetRangeX(), map->GetRangeY(), map->GetResolution(), kCostMapDefaultValue);
...
bool DijkstraBase::TracePath(const MapCell &start, const MapCell &goal, MapCellPath &path) {
    auto step = goal;
    ...
    while (step != start) {
        ...
        // Find next step
        for (auto it = relative_expend_step_cells.begin(); it != relative_expend_step_cells.end(); it++) {
