...
std::string LineSegmentAlgorithm::EvaluatedLine::DebugString(const uint8_t &precision) const {
...
bool LineSegmentAlgorithm::OrthogonalPointToLineDistance(const Line &line, const MapPoint &point, double &distance) {
    if (!line.IsValid()) {
        return false;
    }
    distance = fabs(line.GetA() * point.X() + line.GetB() * point.Y() + line.GetC()) / sqrt(Square(line.GetA()) + Square(line.GetB()));
    return true;
}

bool LineSegmentAlgorithm::PerpendicularPointOnLine(const Line &line, const MapPoint &input_point, MapPoint &perpendicular_point) {
    if (!line.IsValid()) {
        return false;
    }
    auto x = (Square(line.GetB()) * input_point.X() - line.GetA() * line.GetB() * input_point.Y() - line.GetA() * line.GetC()) /
             (Square(line.GetA()) + Square(line.GetB()));
    perpendicular_point.SetX(x);
    auto y = (Square(line.GetA()) * input_point.Y() - line.GetA() * line.GetB() * input_point.X() - line.GetB() * line.GetC()) /
             (Square(line.GetA()) + Square(line.GetB()));
...
bool LineSegmentAlgorithm::LeastSquareFitLine(const MapPoints &points, Line::SPtr &result_line) {
    return LeastSquareFitLine(points.begin(), points.end(), result_line);
}

bool LineSegmentAlgorithm::LeastSquareFitLine(const MapPoints::const_iterator &start_it, const MapPoints::const_iterator &end_it, Line::SPtr &result_line) {
...
bool LineSegmentAlgorithm::AveragePointToLineDistance(const MapPoints &points, const Line &line, double &average_distance) {
    if (points.empty()) {
        return false;
    }
    return AveragePointToLineDistance(points.cbegin(), points.cend(), line, average_distance);
}

bool LineSegmentAlgorithm::AveragePointToLineDistance(const MapPoints::const_iterator &start_it, const MapPoints::const_iterator &end_it, const Line &line,
...
bool LineSegmentAlgorithm::MostValuedLineDegree(const LineSegments &lines, double &degree) {
    if (lines.empty()) {
        return false;
    }

    static auto convert_degree_to_0_90_range = [](const double &input_degree) -> double {
    ...
    for (auto &&line : lines) {
        ...
        if (DoubleEqual(line.GetB(), 0)) {
            ...
            auto degree = convert_degree_to_0_90_range(RadiansToDegrees(atan(-1 * line.GetA() / line.GetB())));
            // ZINFO << "Degree: " << FloatToString(degree, 2);
            index = round(degree);
            if (index < 0 || index >= 180) {
                ZERROR << "Invalid index: " << index << " line: " << line.DebugString(4);
                continue;
            }
            auto it = std::find_if(element_vector.begin(), element_vector.end(), [index](const Element &element) -> bool { return element.first == index; });
    ...
    std::sort(element_vector.begin(), element_vector.end(), [](const Element &a, const Element &b) -> bool { return a.second > b.second; });
...
PointClouds PointCloudLineSegmentExtractor::SeparatePointCloudByPointDistance(const PointCloud &point_cloud) {
    PointClouds result;
    ...
    const double kPointDistanceLimitMin = point_cloud.GetFilterDistance() * kPointsDistanceTolerance;
    ...
    auto found_break_point = [&](const PointCloud::Point &point1, const PointCloud::Point &point2) -> bool {
        auto min_distance = std::min(point1.Distance(), point2.Distance());

        auto points_degree_diff = NormalizeDegree(point1.Degree() - point2.Degree());
        ...
        // cloud point.
        auto points_distance_estimate_from_point_distance = min_distance * fabs(sin(DegreesToRadians(points_degree_diff)));
        auto points_distance_limit_estimate_from_point_distance = min_distance * fabs(sin(DegreesToRadians(kDegreeDiffLimit)));

        const auto point_distance_limit =
            std::max(kPointDistanceLimitMin, std::min(points_distance_estimate_from_point_distance, points_distance_limit_estimate_from_point_distance));
    ...
    while (true) {
        if (tmp_point_cloud == nullptr) {
            tmp_point_cloud.reset(new PointCloud("Point cloud slice " + std::to_string(index)));
        ...
        if (point_it2 == source_points.end()) {
            ...
            if (!result.empty()) {
                ...
                if (FloatEqual(first_point_cloud_points.front().Degree(), source_points.begin()->Degree())) {
                    ...
                    if (!found_break_point(*point_it1, *point_it2)) {
                        ...
                        first_point_cloud_points.insert(first_point_cloud_points.begin(), tmp_point_cloud_points.begin(), tmp_point_cloud_points.end());
...
bool PointCloudLineSegmentExtractor::EstimatePointCloudPointOnLine(const Line &line, const float &point_cloud_degree, MapPoint &estimate_point) {
    if (!line.IsValid()) {
        return false;
    }

    auto radian = DegreesToRadians(point_cloud_degree);
    auto x = -1 * line.GetC() * cos(radian) / (line.GetA() * cos(radian) + line.GetB());
...
LineSegments PointCloudLineSegmentExtractor::ExtractLineSegments(const PointCloud &point_cloud) {
    ...
    while (true) {
        ...
        if (!FindSeedLineSegment(all_points, find_seed_start_it, seed_start_it, seed_end_it, seed_line_segment, config)) {
            ...
            if (GrowFromSeedLineSegment(all_points, seed_start_it, seed_end_it, seed_grow_end_it, seed_line_segment, config)) {
...
bool PointCloudLineSegmentExtractor::GetMostValuedLineDegree(const PointCloud::SPtr &source_point_cloud, double &degree) {
    if (source_point_cloud == nullptr) {
        ZWARN << "Source point cloud invalid.";
        return false;
    }

    auto separate_point_clouds = PointCloudLineSegmentExtractor::SeparatePointCloudByPointDistance(*source_point_cloud);
    ...
    for (auto &&point_cloud : separate_point_clouds) {
        ReadLocker lock(point_cloud->GetLock());
        if (point_cloud->Size() < PointCloudLineSegmentExtractor::kPointCloudPointsCountLimit_) {
            // ZWARN << "Point cloud size: " << point_cloud->Size();
            continue;
        }
        auto &point_cloud_points = point_cloud->GetPointsConstRef();
        saperate_point_cloud_group.emplace_back(
            LineSegment(point_cloud_points.front().X(), point_cloud_points.front().Y(), point_cloud_points.back().X(), point_cloud_points.back().Y()));
        // ZINFO << "Add point cloud group: "
        //       << saperate_point_cloud_group.back().DebugString(2);

        auto _line_segments = PointCloudLineSegmentExtractor::ExtractLineSegments(*point_cloud);
        line_segments.insert(line_segments.end(), _line_segments.begin(), _line_segments.end());
    }
...
PointCloudLineSegmentExtractor::SeedRegionGrowingConfig::SeedRegionGrowingConfig() {
    seed_line_average_deviation_limit_ = 0.07;
    seed_line_improve_percentage_threshold_ = 0.2;
    grow_line_length_threshold_ = 0.4;
}

bool PointCloudLineSegmentExtractor::FindSeedLineSegment(const MapPoints &points, const MapPoints::const_iterator &start_point_it,
                                                         MapPoints::const_iterator &seed_start_point_it, MapPoints::const_iterator &seed_end_point_it,
                                                         LineSegment::SPtr &line_segment, const SeedRegionGrowingConfig &config) {
    // Find seed line segment
    static auto try_to_fit_a_short_line = [](const MapPoints::const_iterator &start_it, const MapPoints::const_iterator &end_it,
                                             LineSegmentAlgorithm::AverageDeviationEvaluatedLine::SPtr &result_line) -> bool {
        ...
        if (LineSegmentAlgorithm::LeastSquareFitLine(start_it, end_it, line)) {
            ...
            if (LineSegmentAlgorithm::AveragePointToLineDistance(start_it, end_it, *line, average_deviation)) {
                result_line.reset(new LineSegmentAlgorithm::AverageDeviationEvaluatedLine(line->GetA(), line->GetB(), line->GetC(), average_deviation));
                return true;
            } else {
                ZWARN;
            }
    ...
    LineSegmentAlgorithm::AverageDeviationEvaluatedLine::SPtr result_line = nullptr;
    ...
    while (true) {
        ...
        LineSegmentAlgorithm::AverageDeviationEvaluatedLine::SPtr tmp_line = nullptr;
        if (try_to_fit_a_short_line(start_it, end_it, tmp_line)) {
            // ZINFO << "Get temp line: " << tmp_line->DebugString(4);
            if (tmp_line->GetMarkValue() < config.seed_line_average_deviation_limit_) {
                if (result_line == nullptr) {
                    // Initialize for line.
                    ...
                    saved_end_it = end_it;
                           ...
                           ((result_line->GetMarkValue() - tmp_line->GetMarkValue()) / result_line->GetMarkValue() >
    ...
    auto ret_begin_point = LineSegmentAlgorithm::PerpendicularPointOnLine(*result_line, *saved_start_it, start_point);
    auto ret_end_point = LineSegmentAlgorithm::PerpendicularPointOnLine(*result_line, *saved_end_it, end_point);
    if (!ret_begin_point || !ret_end_point) {
        ZWARN << "Failed to find point " << ret_begin_point << ", " << ret_end_point;
        return false;
    }

    seed_start_point_it = saved_start_it;
    seed_end_point_it = saved_end_it;
    line_segment.reset(new LineSegment(start_point.X(), start_point.Y(), end_point.X(), end_point.Y()));
...
bool PointCloudLineSegmentExtractor::GrowFromSeedLineSegment(const MapPoints &points, const MapPoints::const_iterator &seed_start_point_it,
                                                             const MapPoints::const_iterator &seed_end_point_it, MapPoints::const_iterator &grow_end_point_it,
                                                             LineSegment::SPtr &seed_line_segment, const SeedRegionGrowingConfig &config) {
    ...
    while (true) {
        ...
        if (new_line_segment == nullptr) {
            ret = LineSegmentAlgorithm::OrthogonalPointToLineDistance(*seed_line_segment, *grow_end_point_it, distance);
        } else {
            ret = LineSegmentAlgorithm::OrthogonalPointToLineDistance(*new_line_segment, *grow_end_point_it, distance);
        }
        if (!ret) {
            ZWARN;
            break;
        ...
        if (distance < config.seed_line_average_deviation_limit_) {
            ...
            if (LineSegmentAlgorithm::LeastSquareFitLine(seed_start_point_it, grow_end_point_it, tmp_line)) {
                double tmp_average_deviation = 0;
                if (LineSegmentAlgorithm::AveragePointToLineDistance(seed_start_point_it, grow_end_point_it, *tmp_line, tmp_average_deviation)) {
                    if (tmp_average_deviation < config.seed_line_average_deviation_limit_) {
                        MapPoint start_point, end_point;
                        auto ret_begin_point = LineSegmentAlgorithm::PerpendicularPointOnLine(*tmp_line, *seed_start_point_it, start_point);
                        auto ret_end_point = LineSegmentAlgorithm::PerpendicularPointOnLine(*tmp_line, *grow_end_point_it, end_point);
                        if (!ret_begin_point || !ret_end_point) {
                            ZWARN << "Failed to find point " << ret_begin_point << ", " << ret_end_point;
                        } else {
                            new_line_segment.reset(new LineSegment(start_point.X(), start_point.Y(), end_point.X(), end_point.Y()));
                            stop_growing = false;
                        }
                    } else {
