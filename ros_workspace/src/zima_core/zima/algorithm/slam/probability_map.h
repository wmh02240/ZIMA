...
class ProbabilityIndexGridMap2D : public DynamicMap2D<ProbabilityIndex> {
  public:
    ...
    class ProbabilityElement {
      public:
        ...
        ProbabilityElement(const float &obstacle_probability, const ProbabilityIndex &next_hit_index, const ProbabilityIndex &next_miss_index)
            : obstacle_probability_(obstacle_probability), next_hit_index_(next_hit_index), next_miss_index_(next_miss_index) {}

        std::string DebugString() const {
            return "Probability: " + FloatToString(obstacle_probability_, 3) + ", next hit index: " + std::to_string(next_hit_index_) +
    ...
    ProbabilityIndexGridMap2D(const std::string &name, const uint16_t &cells_range_x, const uint16_t &cells_range_y, const float &resolution,
                              const Config &config);

    ProbabilityIndexGridMap2D(const ProbabilityIndexGridMap2D &map);

    ProbabilityIndexGridMap2D(const ProbabilityIndexGridMap2D &map, const std::string &name, const bool &reserve = false);
    ...
    bool ResetProbabilityElementVector(const float &hit_probability_on_obstacle, const float &miss_probability_on_obstacle,
                                       const float &max_probability_for_obstacle_in_cell, const float &min_probability_for_obstacle_in_cell,
  ...
  private:
    uint32_t ProbabilityIndexToProbabilityElementsIndex(const ProbabilityIndex &index) const;
    float ProbabilityIndexToProbability(const ProbabilityIndex &index) const;
    ProbabilityIndex ProbabilityToProbabilityIndex(const float &probability) const;
...
class ProbabilityIndexGridMap2DSerializer {
  public:
    ...
    static ZimaProto::ProbabilityMap::PProbabilityIndexGridMap2D ToProto(const ProbabilityIndexGridMap2D::SPtr &map);
    static bool FromProto(ProbabilityIndexGridMap2D::SPtr &map, const ZimaProto::ProbabilityMap::PProbabilityIndexGridMap2D &proto);
};
