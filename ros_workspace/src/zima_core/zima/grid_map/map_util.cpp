...
bool MapConverter::ConvertSlamValueGridMap2DToRoomMap(const SlamValueGridMap2D::SPtr &slam_map, CharGridMap2D::SPtr &room_map) {
    if (slam_map == nullptr) {
    ...
    if (!FloatEqual(slam_map->GetResolution(), room_map->GetResolution())) {
        ZERROR << "Slam map resolution: " << FloatToString(slam_map->GetResolution(), 3)
               << ", room map resolution: " << FloatToString(room_map->GetResolution(), 3);
        return false;
    }
    ...
    room_map->ResetMap(slam_map->GetRangeX(), slam_map->GetRangeY(), slam_map->GetResolution(), room_map->GetDefaultValue());

    WriteLocker room_map_lock(room_map->GetLock());
    auto data_bound = slam_map->GetDataBound();
    auto check_map_value = [&](const SlamValueGridMap2D::DataType &slam_map_value, const CharGridMap2D::DataType &room_map_value) {
        return SlamMapUtil::IsVisiable(slam_map_value) && room_map_value == room_map->GetDefaultValue();
    };
...
bool MapConverter::ConvertSlamValueGridMap2DToCharSlamMap(const SlamValueGridMap2D::SPtr &slam_map, CharGridMap2D::SPtr &char_slam_map) {
    if (slam_map == nullptr) {
        ZERROR << "Slam map ptr invalid.";
        return false;
    }
    ...
    if (!FloatEqual(slam_map->GetResolution(), char_slam_map->GetResolution())) {
        ZERROR << "Slam map resolution: " << FloatToString(slam_map->GetResolution(), 3)
               << ", char slam map resolution: " << FloatToString(char_slam_map->GetResolution(), 3);
        return false;
    }
    // for (auto&& str : slam_map->DebugString()) {
    //   ZISODBG << str;
    // }
    ReadLocker slam_map_lock(slam_map->GetLock());
    char_slam_map->ResetMap(slam_map->GetRangeX(), slam_map->GetRangeY(), slam_map->GetResolution(), char_slam_map->GetDefaultValue());
...
bool MapConverter::ConvertProbabilityGridMap2DToSlamValueGridMap(const ProbabilityIndexGridMap2D::SPtr &probability_map, SlamValueGridMap2D::SPtr &slam_map) {
    ...
    if (!FloatEqual(probability_map->GetResolution(), slam_map->GetResolution())) {
        ZERROR << "Probability map resolution: " << FloatToString(probability_map->GetResolution(), 3)
               << ", slam map resolution: " << FloatToString(slam_map->GetResolution(), 3);
        return false;
    }
    ...
    slam_map->ResetMap(probability_map->GetRangeX(), probability_map->GetRangeY(), probability_map->GetResolution(), slam_map->GetDefaultValue());
    ...
    for (auto x = data_bound.GetMin().X(); x <= data_bound.GetMax().X(); x++) {
        for (auto y = data_bound.GetMin().Y(); y <= data_bound.GetMax().Y(); y++) {
            if (!probability_map->IsUnknown(x, y)) {
                ...
                slam_map->SetValue(x, y,
                                   Clip(static_cast<SlamValueGridMap2D::DataType>(round(probability * 100)), static_cast<SlamValueGridMap2D::DataType>(0),
...
bool SlamMapUtil::OptimizeSlamValueGridMap2DForUser(SlamValueGridMap2D::SPtr &slam_map) {
    if (slam_map == nullptr) {
        ZERROR << "Slam map ptr invalid.";
        return false;
    }
    ...
    WriteLocker slam_map_lock(slam_map->GetLock());
    auto checked_map =
        std::make_shared<CharGridMap2D>("Check map", slam_map->GetRangeX(), slam_map->GetRangeY(), slam_map->GetResolution(), slam_map->GetDefaultValue());
    WriteLocker checked_map_lock(checked_map->GetLock());
    auto data_bound = slam_map->GetDataBound();
    auto check_visiable = [&](const SlamValueGridMap2D::DataType &slam_map_value, const CharGridMap2D::DataType &room_map_value) {
        return SlamMapUtil::IsVisiable(slam_map_value) && room_map_value == checked_map->GetDefaultValue();
    };
    ...
    auto check_unneeded_obstacle = [&](const SlamValueGridMap2D::DataType &slam_map_value, const MapCell &cell) {
        if (SlamMapUtil::IsObstacle(slam_map_value)) {
            for (auto &&check_cell : check_cells) {
                auto _check_cell = cell + check_cell;
                slam_map->GetValue(_check_cell.X(), _check_cell.Y(), check_slam_map_value);
    ...
    for (auto x = data_bound.GetMin().X(); x <= data_bound.GetMax().X(); x++) {
        for (auto y = data_bound.GetMin().Y(); y <= data_bound.GetMax().Y(); y++) {
            slam_map->GetValue(x, y, slam_map_value);
            if (SlamMapUtil::IsUnknown(slam_map_value) && slam_map_value != slam_map->GetDefaultValue()) {
    ...
    for (auto x = data_bound.GetMin().X(); x <= data_bound.GetMax().X(); x++) {
        for (auto y = data_bound.GetMin().Y(); y <= data_bound.GetMax().Y(); y++) {
            ...
            if (check_visiable(slam_map_value, checked_map_value)) {
                ...
                MultiLayersCharGridMap2D::SPtr tmp_multi_layered_map = std::make_shared<MultiLayersCharGridMap2D>(
                    "tmp_multi_layered_map", slam_map->GetRangeX(), slam_map->GetRangeY(), slam_map->GetResolution());
                DijkstraBase::ExpendCondition expend_cond = [&](MultiLayersCharGridMap2D::SPtr _map, MapCell &curr_step, MapCell &next_step) {
                    ...
                    checked_map->GetValue(next_step.X(), next_step.Y(), checked_map_value);
                ...
                DijkstraBase::CellStepCallback step_cb = [&](MultiLayersCharGridMap2D::SPtr map, MapCell &curr_step) {
                    ...
                    checked_map->SetValue(curr_step.X(), curr_step.Y(), NavMap::kRoomA_);
...
bool RoomMapUtil::CheckAndUpdateRoomMap(const SlamValueGridMap2D::SPtr &slam_map, CharGridMap2D::SPtr &room_map) {
    if (room_map == nullptr) {
    ...
    WriteLocker room_map_lock(room_map->GetLock());
    auto data_bound = slam_map->GetDataBound();
    if (!data_bound.Contain(MapCell(room_map->GetDataBound().GetMin().X(), room_map->GetDataBound().GetMin().Y())) ||
        !data_bound.Contain(MapCell(room_map->GetDataBound().GetMax().X(), room_map->GetDataBound().GetMax().Y()))) {
        ZWARN << "Slam map data bound:" << data_bound.DebugString() << " can not cover room map data bound: " << room_map->GetDataBound().DebugString();
        slam_map->Print(__FILE__, __FUNCTION__, __LINE__);
        room_map->Print(__FILE__, __FUNCTION__, __LINE__);
    }
    auto need_to_fullfill = [&](const SlamValueGridMap2D::DataType &slam_map_value, const CharGridMap2D::DataType &room_map_value, const MapCell &check_cell) {
        if (SlamMapUtil::IsVisiable(slam_map_value) && room_map_value != room_map->GetDefaultValue()) {
            CharGridMap2D::DataType check_map_value;
            for (auto &&cell : check_cells) {
                MapCell _check_cell = check_cell + cell;
                room_map->GetValue(_check_cell.X(), _check_cell.Y(), check_map_value);
    ...
    auto need_to_restore = [&](const SlamValueGridMap2D::DataType &slam_map_value, const CharGridMap2D::DataType &room_map_value) {
        return !SlamMapUtil::IsVisiable(slam_map_value) && room_map_value != room_map->GetDefaultValue();
    };
    ...
    auto is_new_area = [&](const SlamValueGridMap2D::DataType &slam_map_value, const CharGridMap2D::DataType &room_map_value) {
        return SlamMapUtil::IsVisiable(slam_map_value) && room_map_value == room_map->GetDefaultValue();
    };
    for (auto x = data_bound.GetMin().X(); x <= data_bound.GetMax().X(); x++) {
        for (auto y = data_bound.GetMin().Y(); y <= data_bound.GetMax().Y(); y++) {
            ...
            if (is_new_area(slam_map_value, room_map_value)) {
                ...
                if (tmp_room_cells.size() >= kMinRoomCellsCount) {
                    ...
                    for (; new_room_value <= static_cast<CharGridMap2D::DataType>(NavMap::kRoomA_ + NavMap::kMaxRoomCount_ - 1); new_room_value++) {
...
bool RoomMapUtil::UpdateRoomsInfo(const CharGridMap2D::SPtr &room_map, RoomsInfo &rooms_info) {
    if (room_map == nullptr) {
        ZERROR << "Room map pointer invalid.";
        return false;
    }
    ...
    rooms_info.clear();
    CharGridMap2D::SPtr tmp_room_map = std::make_shared<CharGridMap2D>(*room_map, "tmp room map", true);
    ...
    for (auto &&room_bound_pair : rooms_bound_info) {
        if (rooms_info.count(room_bound_pair.first) == 0) {
            ...
            rooms_info.at(room_bound_pair.first)->UpdateRoomIndex(room_bound_pair.first);
        } else {
            ZGINFO << "Update room: " << room_bound_pair.first;
        }

        rooms_info.at(room_bound_pair.first)->SetRoomBound(room_bound_pair.second);
        rooms_info.at(room_bound_pair.first)->SetSectionBaseCell(MapCell(room_bound_pair.second.GetMin().X(), room_bound_pair.second.GetMin().Y()));
        uint16_t range_x, range_y;
        if (!GenerateXYRangeForRoom(room_map, room_bound_pair.second, range_x, range_y)) {
            return false;
        }
...
bool RoomMapUtil::SplitRoom(CharGridMap2D::SPtr &room_map, const CharGridMap2D::DataType &target_room_value, const MapCells &split_line,
    ...
    if (split_line.size() != 2) {
        ZERROR << "Split line length invalid: " << split_line.size() << ", only two cells are required";
        return false;
    }
    if (rooms_info.count(target_room_value) == 0) {
        ZERROR << "Selected room " << target_room_value << " is not in rooms info map.";
        return false;
    }

    WriteLocker room_map_lock(room_map->GetLock());

    auto split_line_cells = room_map->GenerateCellsBetweenTwoCells(split_line.front(), split_line.back());

    CharGridMap2D::DataType room_value;
    while (!split_line_cells.empty()) {
        room_map->GetValue(split_line_cells.front().X(), split_line_cells.front().Y(), room_value);
    ...
    while (!split_line_cells.empty()) {
        room_map->GetValue(split_line_cells.back().X(), split_line_cells.back().Y(), room_value);
        if (room_value != target_room_value) {
    ...
    for (auto &&split_line_cell : split_line_cells) {
        tmp_room_map->SetValue(split_line_cell.X(), split_line_cell.Y(), NavMap::kRoomP_);
    }
    ...
    auto get_start_checking_cell = [&](const MapCell &curr_cell, MapCell &start_checking_cell) {
        room_map->GetValue(curr_cell.X(), curr_cell.Y(), room_value);
        tmp_room_map->GetValue(curr_cell.X(), curr_cell.Y(), tmp_room_value);
        if (room_value == target_room_value && tmp_room_value == tmp_room_map->GetDefaultValue()) {
    ...
    for (auto &&split_line_cell : split_line_cells) {
        ...
        for (auto &&check_cell : check_cells) {
            if (get_start_checking_cell(split_line_cell + check_cell, start_checking_cell)) {
                DynamicMapCellBound room_slice_bound(INT_MAX, INT_MIN, INT_MAX, INT_MIN);
                MapCells room_slice_cells;
                ZISODBG << "Start genenrate room from: " << start_checking_cell.DebugString();
                MultiLayersCharGridMap2D::SPtr tmp_multi_layered_map = std::make_shared<MultiLayersCharGridMap2D>(
                    "tmp_multi_layered_map", room_map->GetRangeX(), room_map->GetRangeY(), room_map->GetResolution());
                auto tmp_map_value = tmp_room_map->GetDefaultValue();
                DijkstraBase::ExpendCondition expend_cond = [&](MultiLayersCharGridMap2D::SPtr _map, MapCell &curr_step, MapCell &next_step) {
                    room_map->GetValue(next_step.X(), next_step.Y(), room_value);
                    tmp_room_map->GetValue(next_step.X(), next_step.Y(), tmp_map_value);
                    // ZISODBG << "Check: " << next_step.DebugString()
                    //        << " value: " << checked_map_value;
                    return room_value == target_room_value && tmp_map_value == tmp_room_map->GetDefaultValue();
                };
                // Check nothing.
                DijkstraBase::FinishCondition finish_cond = [&](MultiLayersCharGridMap2D::SPtr _map, MapCell &curr_step) { return false; };
                DijkstraBase::CellStepCallback step_cb = [&](MultiLayersCharGridMap2D::SPtr map, MapCell &curr_step) {
                    ...
                    tmp_room_map->SetValue(curr_step.X(), curr_step.Y(), target_room_value);
                    return;
                };
                ...
                DijkstraBase::IterationStepCallback iter_step_cb = [&](MultiLayersCharGridMap2D::SPtr map, MapCells &cells) {};
                MapCell target_container;
                dijkstra.Search(tmp_multi_layered_map, {start_checking_cell}, target_container, expend_cond, finish_cond, step_cb, iter_step_cb);
                ...
                uint16_t range_x, range_y;
                if (!GenerateXYRangeForRoom(room_map, room_slice_bound, range_x, range_y)) {
                    return false;
                }
                room_slices_deque.emplace_back(RoomSlice(
                    room_slice_cells, std::make_shared<RoomInfo>(target_room_value, room_slice_bound,
                                                                 MapCell(room_slice_bound.GetMin().X(), room_slice_bound.GetMin().Y()), range_x, range_y)));
            }
        }
    }
    ...
    std::sort(room_slices_deque.begin(), room_slices_deque.end(),
              [](const RoomSlice &room_1, const RoomSlice &room_2) { return room_1.first.size() > room_2.first.size(); });
    ...
    rooms_info.emplace(new_room_value,
                       std::make_shared<RoomInfo>(new_room_value, room_slices_deque.front().second->GetRoomBound(),
                                                  room_slices_deque.front().second->GetSectionBaseCell(), room_slices_deque.front().second->GetSectionXRange(),
    ...
    rooms_info.at(target_room_value)->SetSectionBaseCell(MapCell(old_room_bound.GetMin().X(), old_room_bound.GetMin().Y()));
...
bool RoomMapUtil::MergeRoom(CharGridMap2D::SPtr &room_map, const CharGridMap2D::DataType &merge_room_1, const CharGridMap2D::DataType &merge_room_2,
                            RoomsInfo &rooms_info) {
    if (room_map == nullptr) {
        ZERROR << "Room map pointer invalid.";
        return false;
    ...
    ZISODBG << "Try to merge room " << std::string(1, merge_room_1) << " and room " << std::string(1, merge_room_2);
    ...
    for (auto x = data_bound.GetMin().X(); x <= data_bound.GetMax().X(); x++) {
        for (auto y = data_bound.GetMin().Y(); y <= data_bound.GetMax().Y(); y++) {
            ...
            if (room_value == merge_room_1 || room_value == merge_room_2) {
                ...
                CharGridMap2D::DataType another_room_value = (room_value == merge_room_1) ? merge_room_2 : merge_room_1;
                ...
                DijkstraBase::CellStepCallback step_cb = [&](MultiLayersCharGridMap2D::SPtr map, MapCell &curr_step) {
                    ...
                    if (!found_another_room) {
                        CharGridMap2D::DataType check_room_value = room_map->GetDefaultValue();
                        for (auto &&check_cell : check_cells) {
                            MapCell _check_cell = check_cell + curr_step;
                            room_map->GetValue(_check_cell.X(), _check_cell.Y(), check_room_value);
                            if (check_room_value == another_room_value) {
                                ZISODBG << "Found another room " << std::string(1, another_room_value) << " at " << curr_step.DebugString();
                ...
                if (!found_another_room) {
                    ZWARN << "Room " << merge_room_1 << " is not connectted to room " << merge_room_2;
                    return false;
                }

                expend_cond = [&](MultiLayersCharGridMap2D::SPtr _map, MapCell &curr_step, MapCell &next_step) {
                    room_map->GetValue(next_step.X(), next_step.Y(), room_value);
                    // ZISODBG << "Check: " << next_step.DebugString()
                    //        << " value: " << room_value;
                ...
                dijkstra.Search(tmp_multi_layered_map, {another_room_start_cell}, target_container, expend_cond, finish_cond, step_cb, iter_step_cb);

                uint16_t range_x, range_y;
                if (!GenerateXYRangeForRoom(room_map, new_room_bound, range_x, range_y)) {
                    return false;
                }

                ZISODBG << "this room: " << std::string(1, this_room_value) << " this_room_cells.size()" << this_room_cells.size();
                ZISODBG << "another room: " << std::string(1, another_room_value) << " another_room_cells.size()" << another_room_cells.size();
                if (this_room_cells.size() > another_room_cells.size()) {
                    ZINFO << "Merge room " << another_room_value << " into room " << this_room_value;
                    ...
                    rooms_info.at(this_room_value) = std::make_shared<RoomInfo>(
                        this_room_value, new_room_bound, MapCell(new_room_bound.GetMin().X(), new_room_bound.GetMin().Y()), range_x, range_y);
                    for (auto &&cell : another_room_cells) {
                        room_map->SetValue(cell.X(), cell.Y(), this_room_value);
                    }
                } else {
                    ZINFO << "Merge room " << this_room_value << " into room " << another_room_value;
                    ...
                    rooms_info.at(another_room_value) = std::make_shared<RoomInfo>(
                        another_room_value, new_room_bound, MapCell(new_room_bound.GetMin().X(), new_room_bound.GetMin().Y()), range_x, range_y);
...
bool RoomMapUtil::GenerateXYRangeForRoom(const CharGridMap2D::SPtr &room_map, const DynamicMapCellBound &room_bound, uint16_t &range_x, uint16_t &range_y) {
    if (room_map == nullptr) {
        ZERROR << "Room map pointer invalid.";
        return false;
    }
    ...
    auto bigger_point_range =
        std::max(room_bound.GetMax().X() - room_bound.GetMin().X(), room_bound.GetMax().Y() - room_bound.GetMin().Y()) * room_map->GetResolution();
    ...
    if (multiply_factor < 1) {
        ...
        range_x = (bigger_point_range / floor(multiply_factor + 1.0)) / NavMap::GetResolution();
    }
    range_y = range_x;
    return true;
}

float BicubicInterpolator::BiCubicBaseFunctionCalculateWeight(const float &a, const float &input) {
    auto abs_input = fabs(input);
    if (abs_input <= 1) {
        return (a + 2) * pow(abs_input, 3) - (a + 3) * pow(abs_input, 2) + 1;
    } else if (abs_input < 2) {
        return a * pow(abs_input, 3) - 5 * a * pow(abs_input, 2) + 8 * a * abs_input - 4 * a;
    }
    return 0;
}

std::shared_ptr<float> BicubicInterpolator::Interpolate(const ProbabilityIndexGridMap2D::SPtr probability_index_grid_map, const MapPoint &interpolate_point,
                                                        const float &a) {
    ...
    auto x_offset = (interpolate_point.X() - base_point.X()) / probability_index_grid_map->GetResolution();
    auto y_offset = (interpolate_point.Y() - base_point.Y()) / probability_index_grid_map->GetResolution();
    base_cell.AdjustX(-1);
    base_cell.AdjustY(-1);
    ...
    float result = 0;
    auto medium_probability = probability_index_grid_map->GetMediumProbabilityForObstacleInCell();
    ...
    for (auto i = 0; i <= 3; i++) {
        for (auto j = 0; j <= 3; j++) {
            ...
            if (probability_index_grid_map->IsUnknown(cell.X(), cell.Y())) {
                ...
                probability_index_grid_map->GetProbability(cell.X(), cell.Y(), probability);
            }
            if (probability < 0) {
                probability = medium_probability;
            }

            auto x_weight = BiCubicBaseFunctionCalculateWeight(a, x_check_unified_distances[i]);
            auto y_weight = BiCubicBaseFunctionCalculateWeight(a, y_check_unified_distances[j]);
...
FloatValueGridMap2D::SPtr BicubicInterpolator::Interpolate(const ProbabilityIndexGridMap2D::SPtr probability_index_grid_map, const float &scale,
                                                           const float &a) {
    if (probability_index_grid_map == nullptr) {
        ZERROR << "Map pointer invalid, interpolation failed.";
        return nullptr;
    ...
    origin_point_bound.Expend(origin_point_bound.GetMin() - MapPoint(origin_resolution, origin_resolution));
    origin_point_bound.Expend(origin_point_bound.GetMax() + MapPoint(origin_resolution, origin_resolution));

    ZGINFO << "Scale " << FloatToString(scale, 4);
    ZGINFO << "New resolution " << FloatToString(new_resolution, 4);

    FloatValueGridMap2D::SPtr new_map(new FloatValueGridMap2D(probability_index_grid_map->Name() + "(interpolated)",
                                                              ceil(probability_index_grid_map->GetRangeX() * scale),
                                                              ceil(probability_index_grid_map->GetRangeY() * scale), new_resolution, -1));
    WriteLocker lock(new_map->GetLock());
    CharGridMap2D debug_map(probability_index_grid_map->Name() + "(interpolated)", ceil(probability_index_grid_map->GetRangeX() * scale),
                            ceil(probability_index_grid_map->GetRangeY() * scale), new_resolution);
    WriteLocker lock2(debug_map.GetLock());
    for (auto x = origin_point_bound.GetMin().X(); x <= origin_point_bound.GetMax().X(); x += new_resolution) {
        for (auto y = origin_point_bound.GetMin().Y(); y <= origin_point_bound.GetMax().Y(); y += new_resolution) {
            auto new_probability = Interpolate(probability_index_grid_map, MapPoint(x, y));
            ...
            if (new_map->WorldToMap(MapPoint(x, y), cell)) {
                ...
                debug_map.SetValue(cell.X(), cell.Y(), std::to_string(floor(*new_probability * 10))[0]);
            }
        }
    }
    ZGINFO << "Origin point bound: " << origin_point_bound.DebugString() << ", new point bound: " << new_map->GetDataPointBound().DebugString();
    if (FLAGS_debug_enable) {
