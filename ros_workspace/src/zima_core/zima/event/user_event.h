  ...
  public:
    class Operation {
      public:
        ...
        Operation() = delete;
        Operation(const DataType &data_type, const OperationType &operation_type, const NavData::VirtualWall::SPtr &virtual_wall,
                  const NavData::BlockArea::SPtr &block_area, const uint8_t &index)
            : data_type_(data_type), operation_type_(operation_type), virtual_wall_(virtual_wall), block_area_(block_area), index_(index) {}
    ...
    VirtualWallBlockAreaEvent(const Operation::List &operations) : UserEvent(kLabel_), operations_(operations) {}
...
class LocalNavDataEvent : public UserEvent {
  public:
    ...
    LocalNavDataEvent(const OperationType &operation_type, const uint32_t &index) : UserEvent(kLabel_), operation_type_(operation_type), index_(index) {}
