...
const std::string VirtualWallBlockAreaEvent::kLabel_ = "Virtual wall / block area operation";
const std::string VirtualWallBlockAreaEvent::kFileDir_ = "/tmp/virtual_wall_block_area_event/";
const std::string VirtualWallBlockAreaEvent::kAddVirtualWallSuffix_ = ".add_virtual_wall";
const std::string VirtualWallBlockAreaEvent::kRemoveVirtualWallSuffix_ = ".remove_virtual_wall";
const std::string VirtualWallBlockAreaEvent::kUpdateVirtualWallSuffix_ = ".update_virtual_wall";
const std::string VirtualWallBlockAreaEvent::kAddBlockAreaSuffix_ = ".add_block_area";
const std::string VirtualWallBlockAreaEvent::kRemoveBlockAreaSuffix_ = ".remove_block_area";
const std::string VirtualWallBlockAreaEvent::kUpdateBlockAreaSuffix_ = ".update_block_area";
...
VirtualWallBlockAreaEvent::Operation::List VirtualWallBlockAreaEvent::DemoVirtualWallBlockAreaOperation() {
    ...
    auto get_index = [&](const std::string &name, const std::string &suffix, uint8_t &index) -> bool {
        auto index_str = name;
        ...
        ZWARN << "Index " << static_cast<int>(_index) << " too large, should be under " << static_cast<int>(UINT8_MAX);
        return false;
    };
    ...
    for (auto &&name : add_virtual_wall_file_names) {
        ...
        if (loader.LoadData(virtual_wall)) {
            operation_list.emplace_back(Operation::DataType::kVirtualWall, Operation::OperationType::kAdd, virtual_wall, nullptr, 0);
        }
    ...
    for (auto &&name : remove_virtual_wall_file_names) {
        ...
        if (get_index(name, kRemoveVirtualWallSuffix_, index)) {
            operation_list.emplace_back(Operation::DataType::kVirtualWall, Operation::OperationType::kRemove, nullptr, nullptr, index);
        }
        FileSystemHelper::RemoveDirectoryOrFile(kFileDir_ + name);
    }
    for (auto &&name : update_virtual_wall_file_names) {
        ...
        if (get_index(name, kUpdateVirtualWallSuffix_, index)) {
            ...
            NavData::VirtualWall::SPtr virtual_wall;
            if (loader.LoadData(virtual_wall)) {
                operation_list.emplace_back(Operation::DataType::kVirtualWall, Operation::OperationType::kUpdate, virtual_wall, nullptr, index);
            }
        }
        FileSystemHelper::RemoveDirectoryOrFile(kFileDir_ + name);
    }
...
bool VirtualWallBlockAreaEvent::CreateDemoOperationTemplateFile() {
    ...
    auto virtual_wall = std::make_shared<NavData::VirtualWall>(MapPoint(2, 2), MapPoint(2, -2));
    ...
    auto block_area = std::make_shared<NavData::BlockArea>(MapPoint(2, 2), MapPoint(3, 2), MapPoint(3, -2), MapPoint(2, -2));
