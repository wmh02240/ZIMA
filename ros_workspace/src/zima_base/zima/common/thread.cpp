...
ZimaThreadWrapper::ThreadParam::ThreadParam(const std::string &name, const int16_t &bind_cpu_id, const int16_t &priority, const double &cycle_warning_period,
                                            const double &cycle_error_period)
    : thread_name_(name), bind_cpu_id_(bind_cpu_id), priority_(priority), cycle_warning_period_(cycle_warning_period), cycle_error_period_(cycle_error_period) {
}

ZimaThreadWrapper::ThreadParam::~ThreadParam() {}

ZimaThreadWrapper::ZimaThreadWrapper(std::thread &&thread, const ThreadParam &param)
    : access_(std::make_shared<ReadWriteLock>()), param_(param), cycle_state_(kNormal), cycle_start_time_(Time::SystemNow()), last_cycle_time_(0),
      last_print_time_(0), is_marked_exit_(false) {
...
ZimaThreadWrapper::~ZimaThreadWrapper() {
    ...
    if (thread_->joinable()) {
        ZWARN << "Why is it not join by manager thread? Wait for thread \"" << param_.thread_name_ << "\" to join.";
        thread_->join();
...
void ZimaThreadWrapper::SetParam(const ZimaThreadWrapper::ThreadParam &param) { ZERROR << "Never call this funcion.(name: " << param.thread_name_ << ")"; }

bool ZimaThreadWrapper::GetIsMarkedExit() const { return is_marked_exit_.load(); }

void ZimaThreadWrapper::SetIsMarkedExit(const bool &is_marked_exit) { is_marked_exit_.store(is_marked_exit); }
...
void ZimaThreadWrapper::PrintIfNeeded() {
    ...
    case kNormal: {
        if (cycle_time > param_.cycle_warning_period_) {
            ZWARN << "Thread " << param_.thread_name_ << " has run for over " << FloatToString(cycle_time, 4) << "s.";
    ...
    case kWarning: {
        if (cycle_time <= param_.cycle_warning_period_) {
            ...
            ZWARN << "Thread " << param_.thread_name_ << " last cycle run for " << FloatToString(last_cycle_time_, 4) << "s.";
            break;
        }
        if (cycle_time > param_.cycle_error_period_) {
            ZERROR << "Thread " << param_.thread_name_ << " has run for over " << FloatToString(cycle_time, 4) << "s.";
            cycle_state_ = kError;
            last_print_time_ = now;
            break;
        }
    ...
    case kError: {
        if (cycle_time <= param_.cycle_warning_period_) {
            cycle_state_ = kNormal;
            ZERROR << "Thread " << param_.thread_name_ << " last cycle run for " << FloatToString(last_cycle_time_, 4) << "s.";
            break;
        }
        if (cycle_time <= param_.cycle_error_period_) {
            ZERROR << "Thread " << param_.thread_name_ << " last cycle run for " << FloatToString(last_cycle_time_, 4) << "s.";
...
ZimaThreadManager::ZimaThreadManager() {
    ...
    RegisterThread(std::thread(&ZimaThreadManager::MonitorThread, this, thread_param), thread_param);
}

bool ZimaThreadManager::RegisterThread(std::thread &&thread, const ZimaThreadWrapper::ThreadParam &param) {
    ...
    threads_map_.emplace(param.thread_name_, std::make_shared<ZimaThreadWrapper>(std::forward<std::thread>(thread), param));
    ZGINFO << "Register for thread \"" << param.thread_name_ << "\", totally " << threads_map_.size() << " threads running.";
    return true;
}

bool ZimaThreadManager::MarkThreadExited(const std::string &thread_name, const std::string &file, const std::string &function, const uint32_t &line) {
    ReadLocker lock(access_);
    ...
    ZGINFO << file_name << ":" << line << " " << function << ": Mark thread \"" << thread_name << "\" exited.";
    return true;
}
...
bool ZimaThreadManager::WaitForThreadExit(const std::string &thread_name, const double &timeout_s) {
    auto start_time = Time::Now();
    while (true) {
        if (Time::Now() - start_time > timeout_s) {
            ZWARN << "Wait for thread \"" << thread_name << "\" exit timeout(" << DoubleToString(timeout_s, 1) << "s).";
            return false;
        }
...
bool ZimaThreadManager::BindCPUCore(const std::string &thread_name, const uint16_t &cpu_core) {
    if (cpu_core >= cpu_max_core_count_) {
        ZERROR << "Current machine has only " << cpu_max_core_count_ << " cores, unable to bind core " << cpu_core;
        return false;
    }
    cpu_set_t mask;
    CPU_ZERO(&mask);
    CPU_SET(cpu_core, &mask);
    ZGINFO << "Thread \"" << thread_name << "\" try to bind to cpu core " << cpu_core;

    if (-1 == pthread_setaffinity_np(pthread_self(), sizeof(mask), &mask)) {
        ZERROR << "Thread " << thread_name << " bind to cpu core " << cpu_core << " failed.";
        return false;
    }
...
ZimaThreadWrapper::SPtr ZimaThreadManager::GetThreadHandle(const std::string &thread_name) {
    ReadLocker lock(access_);
    if (threads_map_.count(thread_name) == 0) {
        ZWARN << "Thread \"" << thread_name << "\" was not registered.";
        return nullptr;
...
void ZimaThreadManager::MonitorThread(const ZimaThreadWrapper::ThreadParam &param) {
    ZINFO << "Thread start.";
...
bool ZimaProcessManager::SetSystemLevelProcessName(const std::string &system_level_process_name) {
