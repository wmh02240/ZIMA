...
std::string Transform::DebugString() const {
    ...
    msg += "(" + std::to_string(x_) + ", " + std::to_string(y_) + ", " + std::to_string(degree_) + ")";

    return msg;
}

TransformManager::TransformManager() : access_(std::make_shared<ReadWriteLock>()) {
...
bool TransformManager::UpdateTransform(const Transform &transform) {
    ...
    if (transforms_.count(transform.SourceFrame()) != 0) {
        if (transforms_.at(transform.SourceFrame()).TargetFrame() != transform.TargetFrame()) {
            ZERROR << "Changing target frame of " << transform.SourceFrame() << "(" << transform.TargetFrame() << ") is not allowed";
            return false;
        }
    } else {
        transforms_.emplace(transform.SourceFrame(), transform);
        ZGINFO << "Create tf from " << transform.SourceFrame() << " into " << transform.TargetFrame();
    }
...
bool TransformManager::UpdateTransform(const std::string &target_frame, const std::string &source_frame, const MapPoint &offset_in_source_frame) {
    WriteLocker lock(access_);
    if (transforms_.count(source_frame) != 0) {
        if (transforms_.at(source_frame).TargetFrame() != target_frame) {
            ZERROR << "Changing target frame of " << source_frame << "(" << target_frame << ") is not allowed";
            return false;
        }
    }
    ...
    Transform::CoordinateTransformationBA(offset_in_source_frame.X(), offset_in_source_frame.Y(), tf.X(), tf.Y(), DegreesToRadians(tf.Degree()),
                                          offset_in_target_frame_x, offset_in_target_frame_y);

    Transform new_transform(target_frame, source_frame, offset_in_target_frame_x, offset_in_target_frame_y,
                            NormalizeDegree(tf.Degree() + offset_in_source_frame.Degree()));
    transforms_.at(source_frame) = new_transform;
    return true;
}

bool TransformManager::GetTransform(const std::string &target_frame, const std::string &source_frame, Transform &out) const {
    ...
    Transform _out(target_frame, source_frame, transforms_.at(source_frame).X(), transforms_.at(source_frame).Y(), transforms_.at(source_frame).Degree());
    transform_lock.Unlock();

    while (_target_frame != target_frame) {
        // Find source frame.
        if (transforms_.count(_target_frame) == 0) {
            ZWARN << "Can not find transform from " << source_frame << " into " << target_frame << ".";
            return false;
        }
        ...
        Transform::CoordinateTransformationBA(_out.X(), _out.Y(), transforms_.at(_target_frame).X(), transforms_.at(_target_frame).Y(),
                                              DegreesToRadians(transforms_.at(_target_frame).Degree()), new_x, new_y);
        _out.X(new_x);
        _out.Y(new_y);
        _out.Degree(NormalizeDegree(transforms_.at(_target_frame).Degree() + _out.Degree()));
