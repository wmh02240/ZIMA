...
GlobalJsonConfig::GlobalJsonConfig() : access_(std::make_shared<ReadWriteLock>()), global_json_config_(nullptr) { LoadFromFile(); }
...
bool GlobalJsonConfig::GetGlobalConfigObject(const std::string &name, JsonSPtr &object) {
...
bool GlobalJsonConfig::LoadFromFile() {
    ...
    if (!loader.GetJsonFromFile(*json)) {
        ZWARN << "Load json config from file " << kConfigFilePath_ << kConfigFileName_ << " failed.";
        return false;
    }
