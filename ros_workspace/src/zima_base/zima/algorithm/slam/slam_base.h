...
class SlamBase {
  public:
    ...
    class PoseFilter {
      public:
        PoseFilter() = delete;
        PoseFilter(const float &max_linear_distance, const float &max_angle_degree, const float &max_time_interval_s);
        ~PoseFilter() = default;
    ...
    virtual bool StartSlam(const MapPoint &init_pose, const std::string &save_file_name, const std::string &load_file_name);
