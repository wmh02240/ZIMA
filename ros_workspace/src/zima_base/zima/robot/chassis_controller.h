...
class ChassisController {
  public:
    ...
    ChassisController(const Chassis::SPtr &chassis, const float &speed_times = 1.0f);
    ~ChassisController();
    ...
    class PlanInterface {
      public:
        PlanInterface() : access_(std::make_shared<ReadWriteLock>()), is_executed_(false){};
    ...
    class MapInterface {
      public:
        MapInterface() : access_(std::make_shared<ReadWriteLock>()), map_(nullptr){};
     ...
     */
    bool GetCurrentPathExecutionInfo(Steps &past_path, MapPointPath &targets_left);
