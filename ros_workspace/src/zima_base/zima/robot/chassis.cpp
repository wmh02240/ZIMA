...
Chassis::Config::Config(const JsonSPtr &json) : config_valid_(true) {
    ...
    if (json != nullptr) {
        ...
        JsonSPtr config;
        if (GlobalJsonConfig::Instance()->GetGlobalConfigObject(kChassisConfigKey_, config)) {
...
bool Chassis::Config::ParseFromJson(const JsonSPtr &json) {
    ...
    if (!JsonHelper::GetBoolean(*json, kEncircleObstacleOnLeftKey_, encircle_obstacle_on_left_)) {
...
void Chassis::Initialize() {
    ...
    for (auto &&item : config_.device_config_->items()) {
        ...
        if (item.key() == kLeftBumper_ || item.key() == kCenterBumper_ || item.key() == kRightBumper_) {
            Bumper::Config config(NavMap::GetResolution(), NavMap::kRobotCellWidth_2_, radius_, std::make_shared<Json>(item.value()));
            if (!config.config_valid_) {
                ZERROR << "Invalid config:\n" << item.key() << ": " << item.value().dump();
                continue;
            }
        ...
        if (item.key() == kLeftWallSensor_ || item.key() == kRightWallSensor_) {
            ...
            wall_sensor_infos.emplace_back(std::make_shared<WallSensor>(item.key(), config));
        }
        if (item.key() == kGyro_) {
