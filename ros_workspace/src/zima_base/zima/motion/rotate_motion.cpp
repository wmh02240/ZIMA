...
RotateMotion::Config::Config(const float &track_length, const float &wheel_max_speed, const JsonSPtr &json) : MotionConfigBase(json) {
...
bool RotateMotion::Config::ParseFromJson(const float &track_length, const float &wheel_max_speed, const JsonSPtr &json) {
    if (json == nullptr) {
...
RotateMotion::RotateMotion(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const MapPoint &start_odom_pose,
                           const float &rotate_angle, const Config &config, const bool &require_preciseness, const bool &stop_when_reach)
    : MotionBase(half_track_length, max_wheel_speed, min_wheel_speed, config.target_speed_, config.speed_up_step_, config.timeout_),
      rotate_angle_(rotate_angle), accuracy_angle_(config.accuracy_angle_), min_speed_(config.min_speed_), last_odom_pose_(start_odom_pose),
      sum_rotate_angle_(0), require_preciseness_(require_preciseness), stop_when_reach_(stop_when_reach) {
    ...
    if (accuracy_angle_ > fabs(rotate_angle_)) {
        ZGWARN << "Reach target, no need to rotate, accuracy:(" << accuracy_angle_ << "), rotate angle: (" << rotate_angle_ << ")";
        state_ = State::kReachTarget;
        return;
    }
    target_odom_pose_ = start_odom_pose;
    target_odom_pose_.SetDegree(NormalizeDegree(target_odom_pose_.Degree() + rotate_angle_));
    ZGINFO << "Start pose " << start_odom_pose << " target pose: " << target_odom_pose_ << " rotate angle: " << rotate_angle_ << ", speed: " << target_speed_
           << ", speed step: " << FloatToString(speed_up_step_, 4) << (require_preciseness_ ? ", require preciseness." : ".");
}
...
void RotateMotion::CheckState(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
    ...
    if (IsTargetReached(angle_diff)) {
        if (!reached_target_log_displayed_) {
            ZGINFO << "Finish rotate for: " << rotate_angle_ << " degree, current: " << FloatToString(odom_pose.Degree(), 1)
                   << ", sum degree: " << FloatToString(sum_rotate_angle_, 1);
            reached_target_log_displayed_ = true;
        }
        if (stop_when_reach_ && (fabs(current_left_wheel_speed) > 0.01 || fabs(current_right_wheel_speed) > 0.01)) {
...
void RotateMotion::SpeedControl(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
    ...
    if (last_speed_ == nullptr) {
        last_speed_ = std::make_shared<float>((fabs(current_left_wheel_speed) + fabs(current_right_wheel_speed)) / 2);
    }
    ...
    if (stop_when_reach_ && IsTargetReached(angle_diff)) {
        ...
        if (!require_preciseness_) {
            ...
            const float kAdjustSpeedDegreeRange = kAdjustSpeedDegreeMax - kAdjustSpeedDegreeMin;
            ...
            if (degree_left > kAdjustSpeedDegreeMax) {
                ...
                speed_limit = min_speed_ + (target_speed_ - min_speed_) * ((degree_left - kAdjustSpeedDegreeMin) / kAdjustSpeedDegreeRange);
            }
...
bool RotateMotion::IsTargetReached(const float &angle_diff) {
    if (InRange(angle_diff, -accuracy_angle_, accuracy_angle_) || fabs(sum_rotate_angle_) > fabs(rotate_angle_)) {
        return true;
    }
    return false;
}
