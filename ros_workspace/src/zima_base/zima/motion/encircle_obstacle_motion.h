...
class EncircleObstacleMotion : public MotionBase {
  public:
    class Config : public MotionConfigBase {
      public:
        Config() = delete;
        Config(const float &wheel_max_speed, const float &chassis_radius, const MapPoint &wall_sensor_tf, const JsonSPtr &json = nullptr);
        ~Config() = default;

        using SPtr = std::shared_ptr<Config>;

        bool ParseFromJson(const float &wheel_max_speed, const float &chassis_radius, const MapPoint &wall_sensor_tf, const JsonSPtr &json);
    ...
    EncircleObstacleMotion(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const bool &obstacle_on_left,
                           const Config &config);
    ~EncircleObstacleMotion();

    using SPtr = std::shared_ptr<EncircleObstacleMotion>;

    void SetCurrentObsDistance(const MapPoint &pose, const float &current_obstacle_distance);

    void CheckState(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
                    const float &current_right_wheel_speed) override;
    void SpeedControl(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed, const float &current_right_wheel_speed,
                      float &left_wheel_speed, float &right_wheel_speed) override;

    void UpdateDynamicPath(const MapPointPath &path_points_in_odom);

  protected:
    void DistanceBaseSpeedControl(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_speed, float &left_wheel_speed,
                                  float &right_wheel_speed);
    void PathBaseSpeedControl(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_speed, float &left_wheel_speed,
                              float &right_wheel_speed);
