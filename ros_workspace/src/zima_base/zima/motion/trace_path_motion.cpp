...
TracePathMotion::Config::Config(const float &wheel_max_speed, const float &chassis_radius, const JsonSPtr &json) : MotionConfigBase(json) {
...
bool TracePathMotion::Config::ParseFromJson(const float &wheel_max_speed, const float &chassis_radius, const JsonSPtr &json) {
    if (json == nullptr) {
    ...
    if (!JsonHelper::GetFloat(*json, kMinTurnCircleRadiusKey_, min_turn_circle_radius_)) {
        min_turn_circle_radius_ = chassis_radius / 3;
    }
    if (!JsonHelper::GetFloat(*json, kInterpolationDistanceKey_, interpolation_distance_)) {
    ...
    if (!JsonHelper::GetFloat(*json, kMaxReachDistanceKey_, max_reach_distance_)) {
...
TracePathMotion::TracePathMotion(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const MapPointPath &path,
                                 const Config &config)
    : MotionBase(half_track_length, max_wheel_speed, min_wheel_speed, config.target_speed_, config.speed_up_step_, config.timeout_), tracing_last_point_(false),
      path_(path), interpolation_distance_(config.interpolation_distance_),
      // interpolation_path_({}),
      min_turn_circle_radius_(config.min_turn_circle_radius_), max_near_distance_(config.max_near_distance_), max_reach_distance_(config.max_reach_distance_),
    ...
    if (interpolation_distance_ < 0.005) {
        ZWARN << "Interpolation distance " << FloatToString(interpolation_distance_, 4);
        state_ = State::kError;
        return;
    }
    if (min_turn_circle_radius_ < 0) {
        ZWARN << "min_turn_circle_radius_ " << FloatToString(min_turn_circle_radius_, 3);
        state_ = State::kError;
        return;
    }
    ...
    if (path_.size() > 2 || path_.front() != path_.back()) {
        auto point_at_target_vector = MapPoint::GetVector(*(path_.end() - 2), *(path_.end() - 1));
        // Use vector angle as x axis.
        check_reach_target_coordinate_ = std::make_shared<MapPoint>(path_.back().X(), path_.back().Y(), point_at_target_vector.Degree());
    }
    ...
    check_reach_function_ = [&](const MapPoint &world_pose) -> bool {
        if (check_reach_target_coordinate_ == nullptr) {
            ...
            double x, y;
            Transform::CoordinateTransformationAB(world_pose.X(), world_pose.Y(), check_reach_target_coordinate_->X(), check_reach_target_coordinate_->Y(),
...
void TracePathMotion::CheckState(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
...
void TracePathMotion::SpeedControl(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
    ...
    if (InRange(static_cast<double>(degree), -8.0, 8.0)) {
        turn_circle_radius = min_turn_circle_radius_ / fabs(sin(DegreesToRadians(degree / 3)));
        ...
        turn_circle_radius = min_turn_circle_radius_ / fabs(sin(DegreesToRadians(degree * 2)));
...
bool TracePathMotion::ExtendPath(const MapPointPath &path) {
    ...
    ZGINFO << "Target(" << distance << ") pointing at " << target_->DebugString() << "Update " << path.size() << " path points.";
    path_.insert(path_.end(), path.cbegin(), path.cend());
    target_ = path_.begin() + distance;
    ZGINFO << MapPoint::DebugString(path_);
    ZGINFO << "Target(" << distance << ") now pointing at " << target_->DebugString();
    // interpolation_path_ =
