...
class TracePathMotion : public MotionBase {
  public:
    class Config : public MotionConfigBase {
      public:
        Config() = delete;
        Config(const float &wheel_max_speed, const float &chassis_radius, const JsonSPtr &json = nullptr);
        ~Config() = default;

        using SPtr = std::shared_ptr<Config>;

        bool ParseFromJson(const float &wheel_max_speed, const float &chassis_radius, const JsonSPtr &json);
    ...
    TracePathMotion(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const MapPointPath &path, const Config &config);
    // TODO(Austin): What if max/min wheel speed conflict with min_turn_circle_radius...
    ~TracePathMotion();
