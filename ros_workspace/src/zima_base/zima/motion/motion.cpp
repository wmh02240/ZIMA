...
MotionBase::MotionBase(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const float &target_speed,
                       const float &speed_up_step, const float &timeout_sec)
    : access_(std::make_shared<ReadWriteLock>()), half_track_length_(half_track_length), max_wheel_speed_(max_wheel_speed), min_wheel_speed_(min_wheel_speed),
      timeout_sec_(timeout_sec), target_speed_(target_speed), speed_up_step_(speed_up_step), last_speed_(nullptr), handle_slow_down_(false),
    ...
    if (half_track_length_ < 0) {
        ...
        ZWARN << "Invalid speed setting: max(" << FloatToString(max_wheel_speed_, 4) << "), min(" << FloatToString(min_wheel_speed_, 4) << ")";
        state_ = State::kError;
        return;
...
void MotionBase::SlowdownTo(const float &slow_down_speed, const float &slow_down_step) {
    WriteLocker lock(access_);
    if (slow_down_speed < 0) {
        ZWARN << "Slow down speed " << FloatToString(slow_down_speed, 2) << " < 0";
        return;
    } else if (slow_down_speed > target_speed_) {
        ZWARN << "Slow down speed " << FloatToString(slow_down_speed, 2) << " > target speed " << target_speed_;
        return;
    }
...
bool MotionBase::Runable() {
    ...
    default: { // kRunning
        ...
        //       "s";
        if (!DoubleEqual(timeout_sec_, 0) && Time::Now() - start_time_ > timeout_sec_) {
...
void MotionBase::LimitSpeed(float &left_wheel_speed, float &right_wheel_speed) {
    LimitSpeed(left_wheel_speed, right_wheel_speed, min_wheel_speed_, max_wheel_speed_);
}

void MotionBase::LimitSpeed(float &left_wheel_speed, float &right_wheel_speed, const float &limit_speed_min, const float &limit_speed_max) {
    // Narrow speed to limit but maintain turn circle radius.
    while (left_wheel_speed > limit_speed_max || left_wheel_speed < limit_speed_min || right_wheel_speed > limit_speed_max ||
