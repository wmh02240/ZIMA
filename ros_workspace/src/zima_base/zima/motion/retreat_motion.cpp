...
RetreatMotion::Config::Config(const float &wheel_max_speed, const float &retreat_distance, const JsonSPtr &json) : MotionConfigBase(json) {
...
bool RetreatMotion::Config::ParseFromJson(const float &wheel_max_speed, const float &retreat_distance, const JsonSPtr &json) {
    if (json == nullptr) {
...
RetreatMotion::RetreatMotion(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const MapPoint &start_odom_pose,
                             const float &retreat_distance, const Config &config, const bool &require_preciseness, const bool &stop_when_reach)
    : MotionBase(half_track_length, max_wheel_speed, min_wheel_speed, config.target_speed_, config.speed_up_step_, config.timeout_),
      start_odom_pose_(start_odom_pose), retreat_distance_(retreat_distance), min_speed_(config.min_speed_), require_preciseness_(require_preciseness),
...
void RetreatMotion::CheckState(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
...
void RetreatMotion::SpeedControl(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
    ...
    if (stop_when_reach_ && IsTargetReached(odom_pose)) {
        ...
        if (!require_preciseness_) {
            ...
            } else if (fabs(current_speed) > fabs(target_speed_) + fabs(speed_up_step_)) {
            ...
            const float kAdjustSpeedDistanceRange = kAdjustSpeedDistanceMax - kAdjustSpeedDistanceMin;
            ...
            if (distance_left > kAdjustSpeedDistanceMax) {
                ...
                speed_limit = min_speed_ + (target_speed_ - min_speed_) * ((distance_left - kAdjustSpeedDistanceMin) / kAdjustSpeedDistanceRange);
            }
...
float RetreatMotion::RemainDistance(const MapPoint &odom_pose) { return retreat_distance_ - odom_pose.Distance(start_odom_pose_); }

bool RetreatMotion::IsTargetReached(const MapPoint &odom_pose) { return RemainDistance(odom_pose) < 0; }
