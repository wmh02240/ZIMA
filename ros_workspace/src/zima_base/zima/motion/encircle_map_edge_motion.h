...
class EncircleMapEdgeMotion : public MotionBase {
  public:
    class Config : public MotionConfigBase {
      public:
        Config() = delete;
        Config(const float &wheel_max_speed, const float &map_resolution, const JsonSPtr &json = nullptr);
        ~Config() = default;

        using SPtr = std::shared_ptr<Config>;

        bool ParseFromJson(const float &wheel_max_speed, const float &map_resolution, const JsonSPtr &json);
    ...
    EncircleMapEdgeMotion(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const bool &map_edge_on_left,
                          const Config &config);
    ~EncircleMapEdgeMotion();

    using SPtr = std::shared_ptr<EncircleMapEdgeMotion>;

    void SetCurrentMapEdgeDistance(const MapPoint &pose, const float &current_map_edge_distance);
