...
class MotionBase : public DebugBase {
  public:
    MotionBase() = delete;
    MotionBase(const float &half_track_length, const float &max_wheel_speed, const float &min_wheel_speed, const float &target_speed,
    ...
    virtual void SlowdownTo(const float &slow_down_speed, const float &slow_down_step);

  protected:
    virtual void CheckState(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
                            const float &current_right_wheel_speed) = 0;
    virtual void SpeedControl(const MapPoint &world_pose, const MapPoint &odom_pose, const float &current_left_wheel_speed,
                              const float &current_right_wheel_speed, float &left_wheel_speed, float &right_wheel_speed) = 0;

    bool Runable();
    void LimitSpeed(float &left_wheel_speed, float &right_wheel_speed);
    void LimitSpeed(float &left_wheel_speed, float &right_wheel_speed, const float &limit_speed_min, const float &limit_speed_max);
    void Stop();
