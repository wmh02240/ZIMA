...
void Simulator::Run(Chassis::SPtr chassis, TransformManager &tf_manager, const float &duration, NavMap::SPtr map) {
    Transform chassis_tf("", "", 0, 0, 0);
    tf_manager.GetTransform(tf_manager.kOdomFrame_, tf_manager.kRobotFrame_, chassis_tf);
    ...
    auto new_radian = NormalizeRadian(DegreesToRadians(chassis->GetGyro(chassis->kGyro_)->GetDegree()) + radian_speed * duration);

    auto new_x = chassis_tf.X() + distance * cos(new_radian);
    auto new_y = chassis_tf.Y() + distance * sin(new_radian);

    Transform new_chassis_tf(tf_manager.kOdomFrame_, tf_manager.kRobotFrame_, new_x, new_y, RadiansToDegrees(new_radian));
    tf_manager.UpdateTransform(new_chassis_tf);

    chassis->GetGyro(chassis->kGyro_)->SetDegree(RadiansToDegrees(new_radian), Time::Now());
    ...
    MapPoint pose(new_x, new_y, RadiansToDegrees(new_radian));
    {
        ...
        for (; radian < DegreesToRadians(left_bumper->GetCoverRangeMaxDegree()); radian += DegreesToRadians(10)) {
            Transform::CoordinateTransformationBA(chassis->GetRadius() * cos(radian), chassis->GetRadius() * sin(radian), pose.X(), pose.Y(), pose.Radian(), x,
                                                  y);
            if (sensor_map->WorldToMap(x, y, map_x, map_y) && sensor_map->GetValue(map_x, map_y, value) && value != map->kUnknown_) {
                ZINFO << "Left bumper.";
        ...
        for (; radian < DegreesToRadians(right_bumper->GetCoverRangeMaxDegree()); radian += DegreesToRadians(10)) {
            Transform::CoordinateTransformationBA(chassis->GetRadius() * cos(radian), chassis->GetRadius() * sin(radian), pose.X(), pose.Y(), pose.Radian(), x,
                                                  y);
            if (sensor_map->WorldToMap(x, y, map_x, map_y) && sensor_map->GetValue(map_x, map_y, value) && value != map->kUnknown_) {
                ZINFO << "Right bumper.";
    ...
    }
    {
        ...
        for (check_distance = chassis->GetRadius(); check_distance < chassis->GetRadius() + 0.3; check_distance += 0.005) {
            // Change coordinate to map frame.
            Transform::CoordinateTransformationBA(check_distance * cos(radian), check_distance * sin(radian), pose.X(), pose.Y(), pose.Radian(), x, y);
            if (sensor_map->WorldToMap(x, y, map_x, map_y) && sensor_map->GetValue(map_x, map_y, value) && value != map->kUnknown_) {
                break;
            }
        }
        ...
        for (check_distance = chassis->GetRadius(); check_distance < chassis->GetRadius() + 0.1; check_distance += 0.005) {
            // Change coordinate to map frame.
            Transform::CoordinateTransformationBA(check_distance * cos(radian), check_distance * sin(radian), pose.X(), pose.Y(), pose.Radian(), x, y);
