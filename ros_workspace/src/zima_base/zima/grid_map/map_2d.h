...
class DynamicMapCellBound : public BoundingBox<int> {
  public:
    ...
    DynamicMapCellBound(const int &x_min, const int &x_max, const int &y_min, const int &y_max) : BoundingBox<int>(x_min, x_max, y_min, y_max) {}
    ...
    bool OnEastBound(const MapCell &cell) const { return (cell.X() == max_.X() && cell.Y() >= min_.Y() && cell.Y() <= max_.Y()); }

    bool OnSouthBound(const MapCell &cell) const { return (cell.Y() == min_.Y() && cell.X() >= min_.X() && cell.X() <= max_.X()); }

    bool OnWestBound(const MapCell &cell) const { return (cell.X() == min_.X() && cell.Y() >= min_.Y() && cell.Y() <= max_.Y()); }

    bool OnNorthBound(const MapCell &cell) const { return (cell.Y() == max_.Y() && cell.X() >= min_.X() && cell.X() <= max_.X()); }

    bool OnBound(const MapCell &cell) const { return OnEastBound(cell) || OnSouthBound(cell) || OnWestBound(cell) || OnNorthBound(cell); }
};
...
class StaticMap2DBase : public DebugBase {
  public:
     ...
     */
    // TODO(Austin): Stress test for map/world conversion.
    virtual bool WorldToMap(const float &wx, const float &wy, int &x, int &y) const;
    ...
    DynamicMapPointBound GetDataPointBound() const {
        MapPoint min, max;
        MapToWorld(MapCell(data_bound_.GetMin().X(), data_bound_.GetMin().Y()), min);
        MapToWorld(MapCell(data_bound_.GetMax().X(), data_bound_.GetMax().Y()), max);
    ...
    MapCells GenerateCellsBetweenTwoCells(const MapCell &a, const MapCell &b) const;
    ...
    virtual std::vector<std::string> DebugStringWrapper(const DynamicMapCellBound &bound, PrintCellFunc func = nullptr) const;
    virtual std::vector<std::string> DebugStringWrapper(PrintCellFunc func = nullptr) const;
...
class DynamicMap2DBase : public StaticMap2DBase {
  public:
     ...
     * @param  resolution The resolution of the map in meters/cell
     */
    DynamicMap2DBase(const std::string &name, const uint16_t &cells_range_x, const uint16_t &cells_range_y, const float &resolution);
     ...
     */
    // TODO(Austin): Stress test for map/world conversion.
    bool WorldToMap(const float &wx, const float &wy, int &x, int &y) const override;
    ...
    std::vector<std::string> DebugStringWrapper(const DynamicMapCellBound &bound, PrintCellFunc func = nullptr) const override;
    std::vector<std::string> DebugStringWrapper(PrintCellFunc func = nullptr) const override;
  ...
  protected:
    ...
    uint32_t GetIndex(const int &x, const int &y, const int &x_range, const int &y_range) const;
...
template <typename DataTypeT> class DynamicMap2D : public DynamicMap2DBase {
  public:
    using DataType = DataTypeT;
    using OverridePrintCellFunc = std::function<std::string(const DataType &value)>;
     ...
     */
    DynamicMap2D(const std::string &name, const uint16_t &cells_range_x, const uint16_t &cells_range_y, const float &resolution, const DataTypeT &default_value)
        : DynamicMap2DBase(name, cells_range_x, cells_range_y, resolution), default_value_(default_value) {
        // ZERROR << name_;
     ...
     */
    DynamicMap2D(const DynamicMap2D &map) : DynamicMap2DBase(map), default_value_(map.default_value_) {
        // ZERROR << name_;
        data_ = map.data_;
    }

    DynamicMap2D(const DynamicMap2D &map, const std::string &name, const bool &reserve = false)
        : DynamicMap2DBase(map, name), default_value_(map.default_value_) {
        // ZERROR << name_;
        if (reserve) {
            ResetMap();
        } else {
     ...
     */
    virtual void ResetMap(const uint16_t &range_x, const uint16_t &range_y, const float &resolution, const DataType &default_value) {
        WriteLocker lock(access_);
        ...
        optimize_data_size_ = Minimum(static_cast<uint32_t>(optimize_range_x_) * static_cast<uint32_t>(optimize_range_y_), data_size_);
     ...
     * @brief  Reset map data.
     */
    virtual void ResetMap() { ResetMap(range_x_, range_y_, resolution_, default_value_); }
    ...
    bool GetValue(const int &x, const int &y, DataType &value) const {
        ...
        if (!access_->IsLockedByThisThread()) {
            ZWARN << "Map: " << name_ << " is not in read/write mode when accessing (" << x << ", " << y << ")";
            return false;
        }
    ...
    virtual bool SetValue(const int &x, const int &y, const DataType &value) {
        ...
        if (!access_->InWriteByThisThread()) {
            ZWARN << "Map: " << name_ << " is not in write mode when accessing (" << x << ", " << y << ")";
            return false;
        }
        ...
        available_bound_.Reset(data_bound_.GetMax().X() - range_x_, data_bound_.GetMin().X() + range_x_, data_bound_.GetMax().Y() - range_y_,
     ...
     */
    bool CopyRange(const DynamicMap2D &target_map, const DynamicMapCellBound &select_bound) {
        ReadLocker read_lock(target_map.GetLock());
        WriteLocker write_lock(access_);
        for (auto x = select_bound.GetMin().X(); x <= select_bound.GetMax().X(); x++) {
            for (auto y = select_bound.GetMin().Y(); y <= select_bound.GetMax().Y(); y++) {
                DataType value;
    ...
    MapCell GradientMove(const int &init_x, const int &init_y, const uint8_t &iteration_num, const bool &accending, const MapCells &move_step_cells) const {
    ...
    MapCell GradientMoveFor4Direction(const int &init_x, const int &init_y, const uint8_t &iteration_num, const bool &accending) const {
        static const MapCells relative_expend_step_cells{MapCell(1, 0), MapCell(-1, 0), MapCell(0, 1), MapCell(0, -1)};
        return GradientMove(init_x, init_y, iteration_num, accending, relative_expend_step_cells);
    }

    MapCell GradientMoveFor8Direction(const int &init_x, const int &init_y, const uint8_t &iteration_num, const bool &accending) const {
        static const MapCells relative_expend_step_cells{MapCell(1, 0), MapCell(-1, 0), MapCell(0, 1),  MapCell(0, -1),
    ...
    // Glog limits the length of a string to 30000, so we will sepapate string
    // into strings which length is less than 30000.
    virtual std::vector<std::string> DebugString(const DynamicMapCellBound &bound, OverridePrintCellFunc func = nullptr) const {
    ...
    virtual std::vector<std::string> DebugString(OverridePrintCellFunc func = nullptr) const { return DebugString(data_bound_, func); }

    void Print(const DynamicMapCellBound &bound, const std::string &file, const std::string &function, const uint32_t &line) const {
    ...
    void Print(const std::string &file, const std::string &function, const uint32_t &line) const { Print(data_bound_, file, function, line); }
  ...
  private:
    bool ExpendDataSizeIfNecessary(const int16_t &x, const int16_t y) {
        ...
        while ((y <= data_bound_.GetMax().Y() - target_range_y || y >= data_bound_.GetMin().Y() + target_range_y) && target_range_y < range_y_) {
        ...
        while ((x <= data_bound_.GetMax().X() - target_range_x || x >= data_bound_.GetMin().X() + target_range_x) && target_range_x < range_x_) {
        ...
        if (expend_x_data_size || expend_y_data_size) {
            ...
            for (auto _x = data_bound_.GetMin().X(); _x <= data_bound_.GetMax().X(); _x++) {
                for (auto _y = data_bound_.GetMin().Y(); _y <= data_bound_.GetMax().Y(); _y++) {
...
class CharGridMap2D : public DynamicMap2D<char> {
  public:
    CharGridMap2D(const std::string &name, const uint16_t &cells_range_x, const uint16_t &cells_range_y, const float &resolution, const DataType &default_value)
        : DynamicMap2D<char>(name, cells_range_x, cells_range_y, resolution, default_value) {}
    CharGridMap2D(const std::string &name, const uint16_t &cells_range_x, const uint16_t &cells_range_y, const float &resolution)
        : DynamicMap2D<char>(name, cells_range_x, cells_range_y, resolution, kCharGridMap2DDefaultValue_) {}

    CharGridMap2D(const CharGridMap2D &map) : DynamicMap2D<char>(map) {}

    CharGridMap2D(const CharGridMap2D &map, const std::string &name, const bool &reserve = false) : DynamicMap2D<char>(map, name, reserve) {}
    ...
    static void CellPathToPointPath(CharGridMap2D::SPtr map, const MapCellPath &cell_path, MapPointPath &point_path);
    static void PointPathToCellPath(CharGridMap2D::SPtr map, const MapPointPath &point_path, MapCellPath &cell_path);
...
class SlamValueGridMap2D : public Int8GridMap2D {
  public:
    ...
    SlamValueGridMap2D(const std::string &name, const uint16_t &cells_range_x, const uint16_t &cells_range_y, const float &resolution);

    SlamValueGridMap2D(const SlamValueGridMap2D &map) : Int8GridMap2D(map) {}

    SlamValueGridMap2D(const SlamValueGridMap2D &map, const std::string &name, const bool &reserve = false) : Int8GridMap2D(map, name, reserve) {}
    ...
    std::vector<std::string> DebugString(const DynamicMapCellBound &bound, OverridePrintCellFunc func = nullptr) const override;
    std::vector<std::string> DebugString(OverridePrintCellFunc func = nullptr) const override;
...
class FloatValueGridMap2D : public FloatGridMap2D {
  public:
    FloatValueGridMap2D(const std::string &name, const uint16_t &cells_range_x, const uint16_t &cells_range_y, const float &resolution,
                        const float &default_value = 0);

    FloatValueGridMap2D(const FloatValueGridMap2D &map);

    FloatValueGridMap2D(const FloatValueGridMap2D &map, const std::string &name, const bool &reserve = false);
    ...
    std::vector<std::string> DebugString(const DynamicMapCellBound &bound, OverridePrintCellFunc func = nullptr) const override;
    std::vector<std::string> DebugString(OverridePrintCellFunc func = nullptr) const override;
    std::vector<std::string> DebugString(const DynamicMapCellBound &bound, const uint8_t &precision) const;
    std::vector<std::string> DebugString(const uint8_t &precision) const;

    void PrintWithPrecision(const DynamicMapCellBound &bound, const std::string &file, const std::string &function, const uint32_t &line,
                            const uint8_t &precision) const;
    void PrintWithPrecision(const std::string &file, const std::string &function, const uint32_t &line, const uint8_t &precision) const;
...
class DynamicMap2DSerializer {
  public:
    ...
    static bool FromProto(CharGridMap2D::SPtr &map, const ZimaProto::Map::PCharGridMap2D &proto);

    static ZimaProto::Map::PSlamValueGridMap2D ToProto(const SlamValueGridMap2D::SPtr &map);
    static bool FromProto(SlamValueGridMap2D::SPtr &map, const ZimaProto::Map::PSlamValueGridMap2D &proto);
};

class CharGridMap2DLoader : public LocalProtoFileReader {
  public:
    CharGridMap2DLoader() = default;
    explicit CharGridMap2DLoader(const std::string &file_dir, const std::string &file_name) : LocalProtoFileReader(file_dir, file_name, false){};
    ~CharGridMap2DLoader() = default;
...
class CharGridMap2DWriter : public LocalProtoFileWriter {
  public:
    ...
    explicit CharGridMap2DWriter(const std::string &file_dir, const std::string &file_name) : LocalProtoFileWriter(file_dir, file_name, false){};
    ~CharGridMap2DWriter() = default;
...
class SlamValueGridMap2DLoader : public LocalProtoFileReader {
  public:
    ...
    explicit SlamValueGridMap2DLoader(const std::string &file_dir, const std::string &file_name) : LocalProtoFileReader(file_dir, file_name, false){};
    ~SlamValueGridMap2DLoader() = default;
...
class SlamValueGridMap2DWriter : public LocalProtoFileWriter {
  public:
    ...
    explicit SlamValueGridMap2DWriter(const std::string &file_dir, const std::string &file_name) : LocalProtoFileWriter(file_dir, file_name, false){};
    ~SlamValueGridMap2DWriter() = default;

    bool WriteMap(const SlamValueGridMap2D::SPtr &map, const bool &is_binary);
};

template <typename DataTypeT> class StaticMap2D : public StaticMap2DBase {
  public:
    using DataType = DataTypeT;
     ...
     * @param  default_value Default Value
     */
    StaticMap2D(const std::string &name, const float &resolution, const DataTypeT &default_value)
     ...
     * @param map The dynamic map to copy
     */
    StaticMap2D(const StaticMap2D &map) : StaticMap2DBase(map), default_value_(map.default_value_) {
        // ZERROR << name_;
        data_ = map.data_;
    }

    StaticMap2D(const StaticMap2D &map, const std::string &name) : StaticMap2DBase(map, name), default_value_(map.default_value_) {
        // ZERROR << name_;
        data_ = map.data_;
    }
     ...
     * @param  default_value The default value of map.
     */
    virtual void ResetMap(const float &resolution, const DataType &default_value) {
        WriteLocker lock(access_);
     ...
     * @brief  Reset map data.
     */
    virtual void ResetMap() { ResetMap(resolution_, default_value_); }
     ...
     * @param  select_bound The selected cell bound.
     */
    bool CopyRange(const StaticMap2D &target_map, const DynamicMapCellBound &select_bound) {
        ReadLocker read_lock(target_map.GetLock());
        WriteLocker write_lock(access_);
        for (auto x = select_bound.GetMin().X(); x <= select_bound.GetMax().X(); x++) {
            for (auto y = select_bound.GetMin().Y(); y <= select_bound.GetMax().Y(); y++) {
...
class FloatValueGridStaticMap2D : public FloatGridStaticMap2D {
  public:
    FloatValueGridStaticMap2D(const std::string &name, const float &resolution, const float &default_value = 0);

    FloatValueGridStaticMap2D(const FloatValueGridStaticMap2D &map);

    FloatValueGridStaticMap2D(const FloatValueGridStaticMap2D &map, const std::string &name);
