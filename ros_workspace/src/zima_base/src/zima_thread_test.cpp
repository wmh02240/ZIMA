...
void ThreadTest() {
    ...
    ZINFO;
    ZimaThreadWrapper::ThreadParam test1_param("Test1", cpu_max_count - 2, 100, 0.01, 0.3);
    thread_manager->RegisterThread(std::thread(&TestThread, test1_param), test1_param);
    while (true) {
        Time::SleepMSec(20);
    }
}
...
void StressTestThread(const ZimaThreadWrapper::ThreadParam &param) {
    ...
    ZINFO << "Thread \"" << param.thread_name_ << "\" should run for " << run_sec << "s.";
...
void ThreadStressTest() {
    ...
    {
        ZimaThreadWrapper::ThreadParam test1_param(name1, cpu_max_count - 2, 100, -1, -1);
        // ZINFO << static_cast<const void*>(&test1_param);
        thread_manager->RegisterThread(std::thread(&StressTestThread, test1_param), test1_param);
    }
    Time::SleepSec(1);
    std::string name2 = "Stress test2";
    {
        ZimaThreadWrapper::ThreadParam test2_param(name2, cpu_max_count - 4, 100, -1, -1);
        // ZINFO << static_cast<const void*>(&test2_param);
        thread_manager->RegisterThread(std::thread(&StressTestThread, test2_param), test2_param);
    }
