...
void MultiMapTest() {
    ...
    Steps steps2 =
        StepPoint::GenerateStepsBetweenTwoSteps(nav_map.GetFootStepLayer(), StepPoint(MapPoint(0, 0, 0)),
                                                StepPoint(MapPoint(5, 0, 0), {MarkerPoint(MapPoint(3 * resolution, -1 * resolution), nav_map.kBumper_)}));
    ZINFO;
    read_lock.Unlock();
    ...
    read_lock.Lock();
    Steps steps3 =
        StepPoint::GenerateStepsBetweenTwoSteps(nav_map.GetFootStepLayer(), StepPoint(MapPoint(5, 0, 0)),
                                                StepPoint(MapPoint(0, 3, 150), {MarkerPoint(MapPoint(3 * resolution, -1 * resolution), nav_map.kBumper_),
                                                                                MarkerPoint(MapPoint(2 * resolution, 3 * resolution), nav_map.kBumper_)}));
    ZINFO;
    read_lock.Unlock();
    ...
    read_lock.Lock();
    Steps steps4 = StepPoint::GenerateStepsBetweenTwoSteps(nav_map.GetFootStepLayer(), StepPoint(MapPoint(0, 3, 150)), StepPoint(MapPoint(-1.2, 3, 150)));
    ZINFO;
    read_lock.Unlock();
...
void MultiMapPtrTest() {
    ...
    auto mark_footstep = [&](const MapPoint &a, const MapPoint &b) -> void {
        ...
        Steps steps = StepPoint::GenerateStepsBetweenTwoSteps(footstep_layer, StepPoint(a), StepPoint(b));
        read_lock.Unlock();
    ...
    mark_footstep(MapPoint(0, -1 + 1 * resolution), MapPoint(2, -1 + 1 * resolution));
    mark_footstep(MapPoint(0, -1 - 1 * resolution), MapPoint(2, -1 - 1 * resolution));
...
void LoadMapTest() {
    {
        ...
        CharGridMap2D::SPtr test_map(new CharGridMap2D("test", 50, 50, 0.1));
...

    {
        ...
        SlamValueGridMap2D::SPtr test_map(new SlamValueGridMap2D("test", 50, 50, 0.1));
        test_map->Print(__FILE__, __FUNCTION__, __LINE__);
        if (loader.LoadMap(test_map)) {
            test_map->Print(__FILE__, __FUNCTION__, __LINE__);
        } else {
...
void MultiResolutionSlamValueMapTest() {
    SlamValueGridMap2D::SPtr map(new SlamValueGridMap2D("Slam map", 5, 10, 0.05));
    map->Print(__FILE__, __FUNCTION__, __LINE__);
    ...
    MultiResolutionSlamValueGridMap2D multi_resolution_map("Multi resolution slam map", map, 5);
}
...
void WriteMapTest() {
    {
        ...
        SlamValueGridMap2D::SPtr map(new SlamValueGridMap2D("test", 50, 50, 0.1));
        WriteLocker lock(map->GetLock());
        map->SetValue(0, 0, -1);
        map->SetValue(1, 0, 3);
        map->SetValue(2, 0, 30);
...
void MapMemoryTest() {
    ZINFO << "Current memory usage: " << std::to_string(ZimaGetProcessMemoryUsageInKB());
    Time::SleepSec(3);
    ZINFO << "Current memory usage: " << std::to_string(ZimaGetProcessMemoryUsageInKB());
