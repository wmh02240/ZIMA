...
void TimeTest() {
    auto now = Time::Now();
    ZINFO << "Now is: " << std::to_string(now) << " format as " << Time::DebugString(now);
    ...
    ZINFO << "UTC now is: " << std::to_string(utc_now) << " format as " << Time::DebugString(utc_now);
    UTCTime::UpdateUTC(UTCTime::Now() + 3600);
    auto utc_now2 = UTCTime::Now();
    ZINFO << "UTC now2 is: " << std::to_string(utc_now2) << " format as " << Time::DebugString(utc_now2);

    LocalTime::UpdateTimeZoneSec(3600 * 8);
    auto local_time = LocalTime::Now();
    ZINFO << "Local time is: " << std::to_string(local_time) << " format as " << Time::DebugString(local_time);
    LocalTime::UpdateLocalTime(LocalTime::Now() + 3600);
    auto local_time2 = LocalTime::Now();
    ZINFO << "Local time2 is: " << std::to_string(local_time2) << " format as " << Time::DebugString(local_time2);
