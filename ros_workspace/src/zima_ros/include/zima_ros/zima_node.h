...
class ZimaNode {
  public:
    ZimaNode() = delete;
    explicit ZimaNode(const bool &use_simple_slam, const bool &use_sim_time = false);
    ~ZimaNode();
    ...
    void PublishPlanPath(const MapCellPath &path, const double &resolution, const float &x_offset = 0);
