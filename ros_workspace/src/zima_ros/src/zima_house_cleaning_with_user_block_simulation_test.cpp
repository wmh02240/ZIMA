...
void ZimaNode::Run(const ZimaThreadWrapper::ThreadParam &param) {
  ...
  //   CharGridMap2DLoader loader(room_map_file_name);
  //   {
  ...
  operation_data_->AddVirtualWall(
      std::make_shared<NavData::VirtualWall>(MapPoint(1, 1), MapPoint(0, -1)));
  operation_data_->AddVirtualWall(
      std::make_shared<NavData::VirtualWall>(MapPoint(-5, 3), MapPoint(1, -1)));

  operation_data_->AddBlockArea(std::make_shared<NavData::BlockArea>(
      MapPoint(0, 0), MapPoint(1, 0), MapPoint(0.2, 0.2), MapPoint(0, 1)));
  operation_data_->AddBlockArea(std::make_shared<NavData::BlockArea>(
      MapPoint(0, 5), MapPoint(2, 5), MapPoint(2, 6), MapPoint(0, 6)));

  {
    nav_map->GetUserBlockLayer()->Print(__FILE__, __FUNCTION__, __LINE__);
    operation_data_->UpdateUserBlockMapInNavMap();
    nav_map->GetUserBlockLayer()->Print(__FILE__, __FUNCTION__, __LINE__);
  }

  PublishMap(map->GetUserBlockLayer(), kVirtualObsMap);
  ...
  while (!shutdown_request_ &&
    ...
    if (Time::Now() - publish_time > 3) {
      PublishMap(nav_map->GetUserBlockLayer(), kUserAreaMap);
      Time::SleepMSec(50);
      ...
      PublishPath(nav_map->GetPrintLayer(), all_steps);
    }
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "zima_house_cleaning_with_user_block_simulation_test");
