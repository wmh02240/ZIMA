...
void ZimaNode::Run(const ZimaThreadWrapper::ThreadParam &param) {
...

    {
        ...
        CharGridMap2DLoader loader(kInputMapDir, sensor_map_file_name);
        {
            ...
            DijkstraBase::CellStepCallback step_cb = [&](MultiLayersCharGridMap2D::SPtr map, MapCell &curr_step) {
                slam_char_map->SetValue(curr_step.X(), curr_step.Y(), NavMap::kSlamFloor_);
                return;
            };
            ...
            MultiLayersCharGridMap2D::SPtr tmp_multi_layered_map = std::make_shared<MultiLayersCharGridMap2D>(
                "tmp_multi_layered_map", slam_char_map->GetRangeX(), slam_char_map->GetRangeY(), slam_char_map->GetResolution());

            dijkstra.Search(tmp_multi_layered_map, {{0, 0}}, target_container, expend_cond, finish_cond, step_cb, iter_step_cb);
    ...
    auto old_user_select_area_map = nav_map->GetUserSelectAreaLayer();
    auto new_user_select_area_map =
        std::make_shared<CharGridMap2D>(NavMap::kRoomMapName_, old_user_select_area_map->GetRangeX(), old_user_select_area_map->GetRangeY(),
                                        old_user_select_area_map->GetResolution(), NavMap::kUserUnSelected_);
    {
        WriteLocker lock(new_user_select_area_map->GetLock());
        for (auto x = -100; x <= 0; x++) {
            for (auto y = -40; y <= -10; y++) {
                new_user_select_area_map->SetValue(x, y, NavMap::kUserSelected_);
            }
        }
    }

    new_user_select_area_map->Print(__FILE__, __FUNCTION__, __LINE__);

    ZINFO;
    operation_data_->UpdateUserSelectAreaInNavMap(new_user_select_area_map);
    ZINFO;
    {
        auto user_select_area_map = nav_map->GetUserSelectAreaLayer();

        user_select_area_map->Print(__FILE__, __FUNCTION__, __LINE__);
    }
    ...
    MapPoint current_pose(-40 * NavMap::GetResolution(), 42 * NavMap::GetResolution());
    Transform new_chassis_tf(tf.kOdomFrame_, tf.kRobotFrame_, current_pose.X(), current_pose.Y(), current_pose.Degree());
    ...
    for (auto room = NavMap::kRoomA_; room <= NavMap::kRoomD_; room++) {
        rooms_info[room] = std::make_shared<RoomInfo>(room, DynamicMapCellBound(INT_MIN, INT_MAX, INT_MIN, INT_MAX), MapCell(0, 0), 40, 40);
    }
    ...
    while (!shutdown_request_ && auto_cleaning.GetState() != AutoCleaning::State::kFinished) {
...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "zima_user_select_area_cleaning_simulation_test");
    ...
    thread_manager->RegisterThread(std::thread(&zima_ros::ZimaNode::Run, &test, core_thread_param), core_thread_param);
