...
ZimaKobukiNode::ZimaKobukiNode(const std::string &port, const bool &use_simple_slam) : ZimaNode(use_simple_slam) {
    ...
    static_lidar_tf_broadcaster_.sendTransform(zima_ros::GeometryTransformToStampedTransform(
        ...
        zima_ros::TFToGeometryMsgTransform(zima_ros::MapPointToTFTransform(kobuki_chassis_->GetLidar(kobuki_chassis_->kLidar_)->GetTf()))));

    robot_scan_subscriber_ = node.subscribe("/scan", 1, &ZimaKobukiNode::RobotScanCb, this);
...
bool ZimaKobukiNode::ProcessData(uint32_t &map_seq, std::shared_ptr<Timer> &process_map_timer) {
    bool ret = false;
    // Process scan msgs.
    {
        WriteLocker lock(receive_msg_list_lock_);
...
bool ZimaKobukiNode::ChassisStartThread() {
    Chassis::MergedOdomDataCb data_cb = [&](const MergedOdomData::SPtr &data) -> void {
        ...
        while (!odom_data_list_.empty()) {
            if (data->GetTimeStamp() - odom_data_list_.front()->GetTimeStamp() > 0.5) {
...
bool ZimaKobukiNode::ChassisShutdownThread() { return kobuki_chassis_->ShutdownThread(); }
