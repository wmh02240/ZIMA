...
void ZimaNode::Run(const ZimaThreadWrapper::ThreadParam &param) {
    ...
    auto mark_for_obs = [&](const MapPoint &a, const MapPoint &b) -> void {
        ...
        bumper_cells = map->GetFootStepLayer()->GenerateCellsBetweenTwoCells(a_cell, b_cell);
    ...
    while (!shutdown_request_ && section_cleaning.GetState() != SectionCleaning::State::kFinished) {
        ...
        section_cleaning.ChassisSupervise(chassis_, chassis_controller, nav_map, nav_data->GetCurrentRoomInfoRef());
