...
MapPoint OdometryToMapPoint(const nav_msgs::Odometry &odom) {
    ...
    return MapPoint(odom.pose.pose.position.x, odom.pose.pose.position.y, RadiansToDegrees(tf::getYaw(q)));
}
...
MapPoint TFTransformToMapPoint(const tf::Transform &tf) {
    return MapPoint(tf.getOrigin().x(), tf.getOrigin().y(), RadiansToDegrees(tf::getYaw(tf.getRotation())));
}

std::string DebugString(const tf::Transform &tf) {
    return "x: " + std::to_string(tf.getOrigin().x()) + ", y: " + std::to_string(tf.getOrigin().y()) +
           ", degree: " + std::to_string(RadiansToDegrees(tf::getYaw(tf.getRotation())));
}
...
SlamValueGridMap2D::SPtr OccupancyGridToSlamValueGridMap(const std::string &name, const nav_msgs::OccupancyGrid::ConstPtr &msg) {
    SlamValueGridMap2D::SPtr map(new SlamValueGridMap2D(name, msg->info.width, msg->info.height, msg->info.resolution));
     ...
     * index = x * width + y
     */
    MapPoint corner_point(msg->info.origin.position.x, msg->info.origin.position.y);
    ...
    for (auto x = 0u; x < msg->info.height; x++) {
        for (auto y = 0u; y < msg->info.width; y++) {
            ...
            MapPoint point(corner_point.X() + y * msg->info.resolution, corner_point.Y() + x * msg->info.resolution);
            MapCell cell;
...
nav_msgs::OccupancyGrid SlamValueGridMapToOccupancyGrid(const std::string &frame_id, const ros::Time &time, const SlamValueGridMap2D::SPtr &map) {
    ...
    grid.info.origin.position.x = data_bound_min.X() * resolution - 0.5 * resolution;
    grid.info.origin.position.y = data_bound_min.Y() * resolution - 0.5 * resolution;
...
PointCloud::SPtr LaserScanToPointCloud(const sensor_msgs::LaserScan::ConstPtr &scan) {
    ...
    if (scan->ranges.size() != scan->intensities.size()) {
        ZWARN << "Size " << scan->ranges.size() << " vs " << scan->intensities.size();
        return nullptr;
    }
    ...
    for (auto i = 0u; i < scan->ranges.size(); i++) {
        ...
        points.emplace_back(PointCloud::Point(scan->header.stamp.toSec() + scan->time_increment * i, scan->ranges.at(i),
                                              RadiansToDegrees(scan->angle_min + scan->angle_increment * i), scan->intensities.at(i)));
    }
    return new_scan;
}
