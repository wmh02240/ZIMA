...
void ZimaXTarkNode::MainThread() {
    ...
    operation_data_->GetNavMapRef()->GetPrintLayer()->Print(__FILE__, __FUNCTION__, __LINE__);
    ...
    while (!shutdown_request_ && auto_cleaning.GetState() != AutoCleaning::State::kFinished) {
        ...
        if (Time::Now() - publish_time > 3) {
            PublishMap(operation_data_->GetNavMapRef()->GetPrintLayer(), MapType::kPrintMap);
            Time::SleepMSec(50);
            ...
            PublishCleaningInfo(operation_data_->GetOptimizedSlamValueGridMap2DRef(), step_area, operation_data_->GetDuration() / 60);
            PublishUnderLayerMap(operation_data_->GetOptimizedSlamValueGridMap2DRef());
        }
        ...
        serial_->SetVelocityCmdByWheel(left_speed, right_speed, chassis_->GetTrackLength());

        Time::SleepSec(duration);
    }
    operation_data_->GetStopWatchRef().Stop();
    ZINFO << "Finish cleaning, clean for " << operation_data_->GetDuration() << "s.";
    auto step_area = operation_data_->GetNavMapRef()->GetStepAreaSize();
    ZINFO << "Totally clean for " << step_area << "m2, efficiency: " << step_area / operation_data_->GetDuration() * 60;
    ...
    if (false) {
        SlamValueGridMap2DWriter writer(kWriteMapDir, Time::DebugString(Time::SystemNow()) + "slam_map.pbstream");
        writer.WriteMap(operation_data_->GetRawSlamValueGridMap2DRef(), true);
    }
    if (false) {
        CharGridMap2DWriter writer(kWriteMapDir, Time::DebugString(Time::SystemNow()) + "room_map.pbstream");
        auto init_room_map =
            std::make_shared<CharGridMap2D>(NavMap::kRoomMapName_, 1, 1, operation_data_->GetOptimizedSlamValueGridMap2DRef()->GetResolution());
        MapConverter map_converter;
        map_converter.ConvertSlamValueGridMap2DToRoomMap(operation_data_->GetOptimizedSlamValueGridMap2DRef(), init_room_map);
    ...
    while (!shutdown_request_) {
        if (serial_->GetReadValidFramesData(valid_frames_data)) {
            ...
            for (auto &frame_data : valid_frames_data) {
                ...
                Transform new_odom_tf(tf.kOdomFrame_, tf.kRobotFrame_, odom.pose_x_, odom.pose_y_, RadiansToDegrees(odom.angular_z_));
                tf.UpdateTransform(new_odom_tf);

                float left_speed, right_speed;
                serial_->GetVelocityDataForWheel(left_speed, right_speed, chassis_->GetTrackLength());
...
int main(int argc, char **argv) {
    ...
    std::thread publish_odom_thread(&zima_ros::ZimaNode::PublishSensorDataThread, &test);
    std::thread data_process_thread(&zima_ros::ZimaNode::DataProcessThread, &test);
