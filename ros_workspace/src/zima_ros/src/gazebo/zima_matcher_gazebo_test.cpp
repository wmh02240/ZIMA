...
void ZimaGazeboNode::Run(const ZimaThreadWrapper::ThreadParam &param) {
    ...
    // 点云匹配测试、依据雷达点云数据与地图实现机器人的初始定位、重定位
    ...
    while (!shutdown_request_) {
        ...
        if (test_function == kLineSegmentTest && keyboard_event) {
            ...
            if (point_cloud_in_chassis_frame != nullptr) {
                ...
                ZINFO;
                auto separate_point_clouds = PointCloudLineSegmentExtractor::SeparatePointCloudByPointDistance(*point_cloud_in_chassis_frame);
                ZINFO << "Saperate to " << separate_point_clouds.size() << " point clouds";
                ...
                for (auto &&point_cloud : separate_point_clouds) {
                    ...
                    saperate_point_cloud_group.emplace_back(LineSegment(point_cloud_points.front().X(), point_cloud_points.front().Y(),
                                                                        point_cloud_points.back().X(), point_cloud_points.back().Y()));
                    // ZINFO << "Add point cloud group: " << saperate_point_cloud_group.back().DebugString(2);
                ...
                ZINFO;
                PublishLineSegments(saperate_point_cloud_group, "Saperate point clouds");
                PublishLineSegments(line_segments, "Line segments");
        ...
        // 线段提取测试、用于激光雷达点云的分割与特征提取、环境结构分析
        if (test_function == kMatcherTest && slam_grid_map != nullptr && Time::Now() - now > 3) {
            ...
            // matcher.DebugPointCloudInMap(slam_grid_map, init_cell, matcher.ToMapCells(point_cloud_in_map_frame, slam_grid_map));
            // ZWARN << "Match score: " << FloatToString(matcher.SimpleMatchScore(map, init_cell, matcher.ToMapCells(point_cloud_in_map_frame, map)), 4);
            ...
            // PointCloudMatcher::BABSearchParameter parameter(map, init_pose, DynamicMapPointBound(-1, 1, -1, 1), 10, 1, 7, 0.6);
            ...
            // boost_BAB_found = matcher.BoostBABSearch(slam_grid_map, point_cloud_in_chassis_frame, parameter, boost_BAB_result);
            // if (boost_BAB_found) {
            ...
            if (point_cloud_in_chassis_frame != nullptr) {
                ...
                if (LineSegmentAlgorithm::MostValuedLineDegree(line_segments, degree)) {
                    ...
                    PointCloudMatcher::BABSearchParameter optimized_parameter(slam_grid_map, start_pose, bound_in_map_frame, 3, 1, 3, 0.6, true);
                    PointCloudMatcher::MatchResult optimized_BAB_result;
                    bool optimized_BAB_found = matcher.BABSearch(slam_grid_map, point_cloud_in_chassis_frame, optimized_parameter, optimized_BAB_result);
                    if (optimized_BAB_found) {
                        match_pose = optimized_BAB_result.pose_;
                    }
                    ZERROR << "============================================";
                    ZERROR << "Optimize cost " << FloatToString(optimized_BAB_result.match_time_, 2) << "s("
                           << FloatToString(optimized_BAB_result.match_time_ / BAB_result.match_time_ * 100, 2) << "%), non-optimized cost "
                           << FloatToString(BAB_result.match_time_, 2) << "s(100%).";
                } else {
...
int main(int argc, char **argv) {
    ...
    zima::KeyboardListener::CallBackFunc cb = [&](const char &key) -> void { keyboard_event = true; };
