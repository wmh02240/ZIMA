...
void ZimaNode::Run(const ZimaThreadWrapper::ThreadParam &param) {
    ...
    Steps all_steps;
    ZINFO;
    RoomInfo::SPtr room_info(std::make_shared<RoomInfo>(NavMap::kUnknown_, DynamicMapCellBound(INT_MIN, INT_MAX, INT_MIN, INT_MAX), MapCell(0, 0), 40, 40));
    ...
    while (!shutdown_request_ && room_cleaning.GetState() != RoomCleaning::State::kFinished) {
