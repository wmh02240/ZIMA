...
std::string GetROSVersionInfo() {
    ...
    if (version_info.empty()) {
        ...
        version_info += "Zima ros version " + std::string(ZIMA_ROS_MAJOR_VERSION) + "." + std::string(ZIMA_ROS_MINOR_VERSION) + "." +
                        std::string(ZIMA_ROS_PATCH_VERSION) + ", build at " + __DATE__ + " " + __TIME__;
    }
    return version_info;
}
